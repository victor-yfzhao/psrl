...
from pivotrl.trainer.ppo.utils import PivotRL_Role, ResourcePoolManager

logger = logging.getLogger(__file__)
logger.setLevel(os.getenv("PIVOTRL_LOGGING_LEVEL", "INFO"))
...
def _create_rm_manager(config, reward_model):
    ...
    mapping = {
        PivotRL_Role.RewardModel: [f"rm_probe_pool_{i}" for i in range(num_replicas)]
    }
    ...
    reward_worker_cls = ray.remote(max_concurrency=10000)(PivotRL_GenWorker)
    worker_groups = []
    ...
    return PivotRL_RewardModelManager(
        reward_model_name=rm_name,
        config=config,
...
async def _run_probe(config, reward_model, rm_manager) -> dict[str, Any]:
    from pivotrl.workers.reward.reward_loop.registry import load_reward_loop_manager
...
@hydra.main(config_path="../pivotrl/trainer/config", config_name="ppo_trainer", version_base=None)
def main(config) -> None:
    OmegaConf.resolve(config)
    log_dir = Path(OmegaConf.select(config, "rm_probe.log_dir", default="/tmp/pivotrl_rm_probe"))
    log_dir.mkdir(parents=True, exist_ok=True)
    with open_dict(config):
        config.pivotrl.logging_path = str(log_dir)
