...
PIVOTRL_WORKSPACE=$(cd -- "${SCRIPT_DIR}/.." &>/dev/null && pwd)
...
PIVOTRL_GATEWAY_ADDRESS=${PIVOTRL_GATEWAY_ADDRESS:-}

if [[ -z "${VERL_ROUTER_ADDRESS}" ]]; then
  echo "Usage:" >&2
  echo "  VERL_ROUTER_ADDRESS=host:port [VERL_WORKER_ADDRESS=host:port] [PIVOTRL_GATEWAY_ADDRESS=host:port] $0 [n=256] [extra hydra overrides...]" >&2
  echo "" >&2
  echo "Purpose:" >&2
  echo "  Compare the same RM chat-completion requests across:" >&2
  echo "    1. PivotRL internal Ray/DataProto reward router" >&2
  echo "    2. verl reward router HTTP /v1/chat/completions" >&2
  echo "    3. optional direct verl worker HTTP /v1/chat/completions" >&2
  echo "    4. optional PivotRL gateway HTTP /v1/chat/completions" >&2
  exit 2
fi

RUN_ID=$(date +%Y%m%d_%H%M%S)
BASE_OUT_DIR=${OUT_DIR:-"${PIVOTRL_WORKSPACE}/logs/rm_router_compare"}
...
run_case() {
  ...
  if [[ "${mode}" == "http" ]]; then
    OUT_DIR="${case_dir}" bash "${PIVOTRL_WORKSPACE}/scripts/run_rm_path_benchmark.sh" http "${N}" "${address}" "$@" \
      "+bench.label=${label}"
  else
    OUT_DIR="${case_dir}" bash "${PIVOTRL_WORKSPACE}/scripts/run_rm_path_benchmark.sh" "${mode}" "${N}" "$@" \
      "+bench.label=${label}"
  fi
}

run_case "pivotrl_router_internal" "pivotrl_router" "" "$@"
...
if [[ -n "${PIVOTRL_GATEWAY_ADDRESS}" ]]; then
  run_case "pivotrl_gateway_http" "http" "${PIVOTRL_GATEWAY_ADDRESS}" "$@"
else
  echo "[compare_rm_router_paths] skip pivotrl_gateway_http: PIVOTRL_GATEWAY_ADDRESS not set"
fi
