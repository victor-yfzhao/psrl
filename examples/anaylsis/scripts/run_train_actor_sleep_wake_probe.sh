...
MODEL_PATH=${PIVOTRL_TRAIN_ACTOR_PROBE_MODEL_PATH:-${REPO_ROOT}/models/Qwen2.5-32B}
GPUS_PER_NODE=${PIVOTRL_TRAIN_ACTOR_PROBE_GPUS_PER_NODE:-8}
CYCLES=${PIVOTRL_TRAIN_ACTOR_PROBE_CYCLES:-2}
TIMEOUT_S=${PIVOTRL_TRAIN_ACTOR_PROBE_TIMEOUT_S:-1800}
OUTPUT_ROOT=${PIVOTRL_TRAIN_ACTOR_PROBE_OUTPUT_ROOT:-${REPO_ROOT}/logs/train_actor_sleep_wake_probe}
...
if ! [[ "${GPUS_PER_NODE}" =~ ^[1-9][0-9]*$ ]]; then
    echo "Invalid PIVOTRL_TRAIN_ACTOR_PROBE_GPUS_PER_NODE=${GPUS_PER_NODE}" >&2
    exit 2
fi
if ! [[ "${CYCLES}" =~ ^[1-9][0-9]*$ ]]; then
    echo "Invalid PIVOTRL_TRAIN_ACTOR_PROBE_CYCLES=${CYCLES}" >&2
    exit 2
fi
