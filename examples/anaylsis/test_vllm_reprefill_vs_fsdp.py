"""
Compare segmented vLLM logprobs with PivotRL-style trainer recomputation.
...
PivotRL-style padded tensors and metadata. No optimizer is created and no model
...
Example:
  ...
  CUDA_VISIBLE_DEVICES=0 PYTHONPATH=. python \
      ...
      --output-dir /tmp/qwen2.5-7b-reprefill-vs-pivotrl
    ...
    try:
        for prompt_id, (prompt, prompt_token_ids) in enumerate(resolved_prompts):
            if args.include_continuous_baseline:
                pivotrl_logger.info("[reprefill_exp] Generating continuous case for prompt %d.", prompt_id)
...
def _build_pivotrl_batch_tensors(
    ...
    *,
    ...
    """
    Build the padded tensors consumed by PivotRL's actor recompute path.
...
def _run_fsdp_phase(
    ...
    actor = None
    try:
        pivotrl_logger.info("[reprefill_exp] Loading trainer checkpoint for PivotRL-style FSDP2 recompute.")
        ...
        tensors = _build_pivotrl_batch_tensors(
            all_cases,
        ...
        if log_probs.shape != (
            ...
            raise RuntimeError(
                f"PivotRL actor returned logprob shape {tuple(log_probs.shape)}, expected "
                f"{(len(all_cases), args.response_length)}"
            )
        if entropies is None or entropies.shape != log_probs.shape:
            raise RuntimeError("PivotRL actor did not return response-aligned entropy")
        ...
        metadata = {
            ...
            "full_pivotrl_runtime_differences": [
                ...
                "PivotRL weight-arena and parameter offload integrations are not enabled",
            ],
...
def _plot_results(
    ...
    os.environ.setdefault("MPLCONFIGDIR", "/tmp/pivotrl-matplotlib")
    import matplotlib
...
def _self_test() -> None:
    ...
    pivotrl_logger.info("[reprefill_exp] Self-test passed.")
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--output-dir",
        default="/tmp/qwen2.5-7b-reprefill-vs-pivotrl",
    )
...
def _validate_args(args: argparse.Namespace) -> None:
    ...
    if args.temperature <= 0:
        raise ValueError("--temperature must be positive, matching PivotRL trainer recompute requirements")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        pivotrl_logger.error("[reprefill_exp] Invalid arguments: %s.", exc)
        return 2

    import torch

    if not torch.cuda.is_available() or torch.cuda.device_count() == 0:
        pivotrl_logger.error("[reprefill_exp] No CUDA device is visible.")
    ...
    try:
        ...
        pivotrl_logger.info("[reprefill_exp] Full report written to %s.", report_path)
        pivotrl_logger.info(
        ...
        pivotrl_logger.exception("[reprefill_exp] Experiment failed.")
        return 1
