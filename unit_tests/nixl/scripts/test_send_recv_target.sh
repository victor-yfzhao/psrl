...
PYTHONUNBUFFERED=1 python ${PIVOTRL_WORKSPACE}/pivotrl/unit_tests/nixl/test_send_recv.py --ip ${IP} --mode target --cuda ${GPU_ID}
# PYTHONUNBUFFERED=1 python ${PIVOTRL_WORKSPACE}/pivotrl/unit_tests/nixl/test_send_recv_model.py --ip ${IP} --mode target --cuda ${GPU_ID} --model_path ${PIVOTRL_WORKSPACE}/models/Qwen2.5-Math-7B
