...
from pivotrl.environments.base import ConversationType, Environment, EnvStepOutput
from pivotrl.tools.base import ToolGroup, initialize_tools_from_config
...
class ToolEnvironment(Environment[ConversationType, ToolAction]):
    ...
    def init_class(cls, config: DictConfig, **kwargs) -> None:
        ...
        pivotrl_logger.addHandler(FileOnlyHandler(config.pivotrl.logging_path, log_prefix))
        pivotrl_logger.info("ToolEnvironment logger initialized for tool calls.")
    ...
    async def _call_tool(self, tool_call: dict) -> dict[str, str]:
        ...
        try:
            ...
            pivotrl_logger.warning(f"Error when executing tool: {e}")
            return (
