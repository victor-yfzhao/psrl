...
# Custom Environments & AgentData in PivotRL

This document explains **what an Environment and AgentData are in PivotRL**, and how to **create your own** and plug them into `multi_turn_agent_loop` **without changing the agent loop code**.
...
In PivotRL, `multi_turn_agent_loop` calls:
...
In PivotRL, `multi_turn_agent_loop` calls on AgentData:
...
Create a new file, e.g. `pivotrl/environments/my_env.py`:
...
from pivotrl.environments.base import Environment, EnvStepOutput
...
Create a new file, e.g. `pivotrl/workers/agent_loop/agent_data/my_agent_data.py`:
...
from pivotrl.environments.base import ConversationType, Environment
from pivotrl.workers.agent_loop.agent_data.base import AgentData, Trajectory
