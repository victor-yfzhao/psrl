...
def resolve_weight_fingerprint_options(
    ...
    *,
    ...
    if not weight_fingerprint_flow_enabled(pivotrl_config, flow=flow):
        return None
    config = OmegaConf.select(pivotrl_config, "nixl.weight_verification", default=None)
...
def weight_fingerprint_flow_enabled(pivotrl_config: Any, *, flow: str) -> bool:
    config = OmegaConf.select(pivotrl_config, "nixl.weight_verification", default=None)
