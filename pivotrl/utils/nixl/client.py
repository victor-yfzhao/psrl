...
from pivotrl.utils.logger import DualOutputHandler, deprecated, get_worker_info, log_tensor
from pivotrl.utils.nixl.comm_plan import NIXLCommPlan
from pivotrl.utils.nixl.fingerprint import fingerprint_tensor_mapping, tensor_mapping_signature
from pivotrl.utils.nixl.meta_buffer import MetaBuffer
from pivotrl.utils.nixl.network_topology import get_local_gpu_id, get_local_ip
...
class NIXLStorageClient:
    def __init__(
        ...
        if self.enable_tms_for_temp_buffers:
            ...
            apply_tms_patch()
            pivotrl_logger.info(f"NIXLStorageClient {self.client_name} enabled TMS for temporary buffers.")
        ...
        if logging_path is not None:
            ...
            pivotrl_logger.info(f"NIXLStorageClient {self.client_name} initialized.")
    ...
    def register_local_tensors(
        ...
        with tms_ctx:
            ...
            pivotrl_logger.info(f"{self.client_name} all local tensors are registered.")
    ...
    def deregister_local_tensors(self):
        ...
        pivotrl_logger.debug(f"{self.client_name} deregistered all local tensors.")
    ...
    def wait_for_update_infos(self, expected_agents: int, timeout: float = 600.0):
        ...
        """
        assert self._is_connected, "Not connected to server"
        pivotrl_logger.info(f"{self.client_name}: Waiting for {expected_agents} updated client infos...")
        start = time.time()
    ...
    def _ensure_client_info_fetched(self, target_client: str):
        ...
        try:
            ...
            pivotrl_logger.error(f"Error adding remote agent {target_client}: {e}")
            raise e
