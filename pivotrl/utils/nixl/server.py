...
from pivotrl.utils.nixl.comm_plan import CommunicationPlanner, NIXLCommPlan
...
class NIXLMetaServer:
    ...
    def wait_for_client_temp_mappings(self, expected_agents: int = 1, timeout: float = 600.0):
        ...
        """
        pivotrl_logger.info(f"Waiting for {expected_agents} agents to send temp mappings...")
    ...
    def make_comm_plan(self):
        ...
        pivotrl_logger.info("Making communication plan...")
        start = time.time()
        try:
            ...
            raise
        pivotrl_logger.info(f"Communication plan made after {time.time() - start} seconds.")
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    for pending in futures:
                        pending.cancel()
                    pivotrl_logger.exception("Failed to notify NIXL agent %s", agent_name)
                    raise
        ...
        with ThreadPoolExecutor(max_workers=max_workers, thread_name_prefix="nixl-meta-temp-notify") as executor:
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    for pending in futures:
                        pending.cancel()
                    pivotrl_logger.exception("Failed to notify NIXL temp mappings to agent %s", agent_name)
                    raise
    ...
    def wait_for_update_infos(self, expected_agents: int, timeout: float = 600.0):
        ...
        """
        pivotrl_logger.debug(f"Waiting for {expected_agents} agents to send updated client infos...")
        start = time.time()
        already_recved_agents = set()
        while len(already_recved_agents) < expected_agents:
            notifs = self.agent.get_new_notifs()
            for agent_name, notif_list in notifs.items():
                for notif in notif_list:
                    try:
                        ...
                        pivotrl_logger.error(f"Failed to parse updated client infos from agent {agent_name}: {e}")
                        raise
            if time.time() - start > timeout:
                raise TimeoutError("Timeout waiting for agents.")
            time.sleep(0.1)
