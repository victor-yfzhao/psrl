...
def _log_with_caller_info(pivotrl_logger: logging.Logger, level: int, message: str):
    """Log a message with the caller's file and line information."""
    # Check if the logger is enabled for the given level
    if not pivotrl_logger.isEnabledFor(level):
        return

    # Try to use stacklevel to get the caller's file and line information
    try:
        pivotrl_logger.log(level, message, stacklevel=3)
    except TypeError:
        ...
        try:
            ...
            record = pivotrl_logger.makeRecord(pivotrl_logger.name, level, pathname, lineno, message, (), None)
            pivotrl_logger.handle(record)
        finally:
            del frame
...
def log_dual_events(
    message: str,
    pivotrl_logger: logging.Logger,
    level: int = logging.INFO,
    event_type: EventType = EventType.OTHER,
):
    start_time = time.time()
    log_begin_event(message, pivotrl_logger, level, event_type)  # Log with label when entering
    try:
        yield  # Execute code within the with block
    finally:
        end_time = time.time()
        log_end_event(message, pivotrl_logger, level, event_type, end_time - start_time)  # Log end tag when exiting
...
def log_single_event(
    ...
    _log_with_caller_info(pivotrl_logger, level, f"[Single Event] {event_type.value} - {message}")
...
def log_begin_event(
    ...
    _log_with_caller_info(pivotrl_logger, level, f"[Begin Event] {event_type.value} - {message}")
...
def log_end_event(
    ...
    if duration is None:
        _log_with_caller_info(pivotrl_logger, level, f"[End Event] {event_type.value} - {message}")
    else:
