"""Helpers for PivotRL uid vs vLLM V1 internal request IDs.

vLLM V1 ``InputProcessor.assign_request_id`` rewrites externally supplied IDs to
``{external_req_id}-{random_uuid()[:8]}`` (8 hex chars) for uniqueness. PivotRL
...
def parse_pivotrl_uid_from_request_id(request_id: int | str) -> int | str:
    """Map a request id (PivotRL uid or vLLM internal id) to the PivotRL active-task key.

    Examples:
        973 -> 973
        "973" -> 973
        "973-8d745705" -> 973  (vLLM internal id; external part is PivotRL uid)
    """
...
def canonical_pivotrl_request_id(request_id: int | str) -> str:
    """Return the router key for a logical or vLLM-internal request ID."""
    return str(parse_pivotrl_uid_from_request_id(request_id))
