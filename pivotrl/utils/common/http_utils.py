    ...
    while retry_count < max_retries:
        try:
            ...
            if retry_count >= max_retries:
                pivotrl_logger.info(f"Max retries ({max_retries}) reached, failing... (url={url})")
                raise e
...
async def post(url, payload, max_retries=60):
    ...
    if _http_client is None:
        raise RuntimeError(
            "HTTP client is not initialized. Call pivotrl.utils.common.http_utils.init_http_client() "
