...
from pivotrl.utils.dataset.utils import create_rl_dataset, create_rl_sampler
from pivotrl.utils.logger import DualOutputHandler, log_data_protocol
...
class DataProcessor:
    ...
    def build_train_dataloader(self) -> None:
        ...
        if self.config.pivotrl.redundant_rollout.enable:
            total_batch_size = self.config.pivotrl.redundant_rollout.redundant_global_batch_size
        else:
    ...
    #     pivotrl_logger.info(f"Train dataloader saved to {dataloader_local_path}")
    ...
    #     pivotrl_logger.info(f"Train dataloader loaded from {dataloader_local_path}")
    ...
    def save_train_dataloader(self, dataloader_local_paths: list[str]) -> None:
        ...
        pivotrl_logger.info(f"Train dataloaders saved to {dataloader_local_paths}")
    ...
    def load_train_dataloader(self, dataloader_local_paths: list[str]) -> None:
        ...
        pivotrl_logger.info(f"Train dataloaders loaded from {dataloader_local_paths}")
    ...
    def get_train_next(self):
        ...
        #     pivotrl_logger.debug("Train dataloader iterator exhausted.")
        #     epoch += 1
        #     if epoch == self.config.trainer.total_epochs:
        #         pivotrl_logger.info("All training epochs completed, stopping data processing.")
        #         raise
        #     self.train_dataloader_iter = iter(self.train_dataloader)
        #     data = next(self.train_dataloader_iter)
        # return data
    ...
    def get_val_next(self):
        ...
        # except StopIteration:
        #     pivotrl_logger.info("Validation dataloader iterator exhausted.")
        #     self.val_dataloader_iter = iter(self.val_dataloader)
        #     raise
        # return data
        ...
        while self._val_dataloader_idx < num_dataloaders:
            ...
            try:
                ...
                pivotrl_logger.info(f"Validation dataloader {i} iterator exhausted.")
    ...
    def _process_data(self):
        ...
        while not self.stop_data_process:
            try:
                ...
                pivotrl_logger.debug(f"Generating {batch_size} requests with rollout n {self.rollout_n}")
                if self.rollout_n > 1:
                ...
                pivotrl_logger.debug(f"StopIteration encountered, current epoch: {curr_epoch}/{total_epochs}")
                if curr_epoch >= total_epochs:
                    pivotrl_logger.info("All training epochs completed, stopping data processing.")
                    self.stop_data_process = True
                else:
                    pivotrl_logger.debug("Reinitializing train_dataloader_iters for next epoch")
                    self.train_dataloader_iters = [iter(dataloader) for dataloader in self.train_dataloaders]
        ...
        pivotrl_logger.info("Data processing stopped, sending shutdown signal.")
