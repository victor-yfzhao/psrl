...
from pivotrl.utils.elastic_rm.dummy_scaling_policy import DummyScalingPolicy
...
from pivotrl.utils.elastic_rm.scaling_policy import InstanceSignal, ScalingPolicy
from pivotrl.utils.logger import DualOutputHandler, FileOnlyHandler
...
monitor_logger.setLevel(os.getenv("PIVOTRL_LOGGING_LEVEL", "WARN"))
...
class ElasticExecutor:
    def __init__(
        self,
        config,
        roles: list[tuple[PivotRL_Role, str]],
        coordinators: dict[PivotRL_Role, dict[str, ray.actor.ActorHandle]],
        ...
        self.instances_status_flags: dict[PivotRL_Role, dict[str, dict[int, InstanceStatus]]] = {}
        self.instances_engine_stats: dict[PivotRL_Role, dict[str, dict[int, dict | None]]] = {}
        # Per-instance pool-local bundle_range (start, end); used for cross-role conflict checks.
        self.instance_bundle_mappings: dict[PivotRL_Role, dict[str, dict[int, dict[str, object]]]] = {}
        self.bundle_to_instances: dict[tuple[str, int], set[tuple[PivotRL_Role, str, int]]] = {}
        ...
        self.router_backlog_by_role: dict[PivotRL_Role, int] = {}
        self.router_backlog_summary_by_role: dict[PivotRL_Role, dict[str, int]] = {}
        self.request_level_snapshots_by_role: dict[PivotRL_Role, object] = {}
        ...
        monitor_logger.addHandler(FileOnlyHandler(self.config.pivotrl.logging_path, "ElasticMonitor"))

    def register_instances(self, role_name: PivotRL_Role, model_name: str, num_instances: int):
        self.instances_status_flags.setdefault(role_name, {}).setdefault(model_name, {})
        self.instances_engine_stats.setdefault(role_name, {}).setdefault(model_name, {})
        self.instance_bundle_mappings.setdefault(role_name, {}).setdefault(model_name, {})
    ...
    def _set_instance_immunity(self, role_name: PivotRL_Role, model_name: str, instance_id: int) -> None:
    ...
    def _is_instance_in_immunity(self, role_name: PivotRL_Role, model_name: str, instance_id: int) -> bool:
    ...
    def set_instance_immunity(self, role_name: PivotRL_Role, model_name: str, instance_id: int) -> None:
    ...
    def _snapshot_instance_statuses(
        self,
        instances: list[dict],
    ) -> dict[tuple[PivotRL_Role, str, int], InstanceStatus | None]:
        snapshot: dict[tuple[PivotRL_Role, str, int], InstanceStatus | None] = {}
    ...
    def _count_instance_status_transitions(
        self,
        before: dict[tuple[PivotRL_Role, str, int], InstanceStatus | None],
        *,
    ...
    @staticmethod
    def _instance_key(role_name: PivotRL_Role, model_name: str, instance_id: int) -> tuple[PivotRL_Role, str, int]:
        return (role_name, model_name, int(instance_id))

    def _normalize_instance_entries(self, instances: list[dict] | None) -> list[dict]:
        normalized: list[dict] = []
        seen: set[tuple[PivotRL_Role, str, int]] = set()
    ...
    def _get_instance_pool_id(self, role_name: PivotRL_Role, model_name: str, instance_id: int) -> str:
    ...
    def _get_instance_bundle_indices(self, role_name: PivotRL_Role, model_name: str, instance_id: int) -> list[int]:
        mapping = self.instance_bundle_mappings.get(role_name, {}).get(model_name, {}).get(instance_id, {})
    ...
    def _remove_instance_from_bundle_reverse_index(self, role_name: PivotRL_Role, model_name: str, instance_id: int):
    ...
    def _add_instance_to_bundle_reverse_index(self, role_name: PivotRL_Role, model_name: str, instance_id: int):
        target_key = self._instance_key(role_name, model_name, instance_id)
        pool_id = self._get_instance_pool_id(role_name, model_name, instance_id)
        for bundle_idx in self._get_instance_bundle_indices(role_name, model_name, instance_id):
            self.bundle_to_instances.setdefault((pool_id, bundle_idx), set()).add(target_key)

    def _has_other_role_awaken_on_shared_bundle(self, role_name: PivotRL_Role, model_name: str, instance_id: int) -> bool:
        pool_id = self._get_instance_pool_id(role_name, model_name, instance_id)
        for bundle_idx in self._get_instance_bundle_indices(role_name, model_name, instance_id):
        ...
        def _log_background_task_done(task_name: str):
            def _callback(task: asyncio.Task) -> None:
                try:
                    ...
                    pivotrl_logger.warning("elastic_rm background task cancelled: %s", task_name)
                except Exception:
                    pivotrl_logger.exception("elastic_rm background task failed: %s", task_name)
                else:
                    pivotrl_logger.warning("elastic_rm background task exited unexpectedly: %s", task_name)
        ...
        while not self.stop_monitor:
            try:
                ...
                if decision.actions:
                    ...
                    pivotrl_logger.info("elastic_rm trainer_waiting_hint=%s", self.trainer_waiting_hint)
                    for action in decision.actions:
                        task = {
                ...
                pivotrl_logger.exception("elastic_rm _monitor_loop iteration failed; will retry after sleep.")
        ...
        while not self.stop_scale_up:
            ...
            try:
                ...
                if not instances_to_scaled_up:
                    pivotrl_logger.warning("No instances can be scaled up for role %s", role_need_to_scale_up)
                    continue
                ...
                pivotrl_logger.info("elastic_rm scale_up_handler decision_id=%s post_scale_up_abort done", decision_id)
            except Exception:
        ...
        while not self.stop_scale_down:
            ...
            try:
                ...
                if not instances_to_scaled_down:
                    pivotrl_logger.warning("No instances can be scaled down for role %s", role_need_to_scale_down)
                    continue
                pivotrl_logger.info(
                ...
                pivotrl_logger.info("elastic_rm scale_down_handler decision_id=%s sleep_targets done", decision_id)
            except Exception:
                pivotrl_logger.exception(
    ...
    def _mark_decision_action_finished(self, decision_id: int | None):
        ...
        if not self._decision_pending_action_counts:
            pivotrl_logger.info("elastic_rm decision_id=%d execution completed.", decision_id)
    ...
    def _record_policy_migration_observation(self, sleep_elapsed_s: float, wake_elapsed_s: float) -> None:
        ...
        try:
            ...
            pivotrl_logger.exception("elastic_rm failed to record policy migration observation.")
    ...
    def _maybe_log_instance_signals(self, signals: list[InstanceSignal]):
        ...
        monitor_logger.info("------------------------------------------------------------")
        pivotrl_logger.info("Router backlog summary: by_role=%s", router_backlog_log)
    ...
    async def _interrupt_vllm_waiting_loop(self):
        while not self.stop_interrupt_vllm_waiting:
            try:
                ...
                pivotrl_logger.exception("elastic_rm vLLM waiting interrupt loop failed; will retry after sleep.")
            await asyncio.sleep(self._interrupt_vllm_waiting_interval_s)

    async def _interrupt_vllm_waiting_for_role(self, role_name: PivotRL_Role, model_name: str):
        role_status = self.instances_status_flags.get(role_name, {}).get(model_name, {})
        awaken_instance_ids = [
    ...
    async def _scale_down_instances(self, instances_to_scaled_down: list[dict]) -> bool:
        grouped: dict[tuple[PivotRL_Role, str, int | None], list[int]] = defaultdict(list)
    ...
    async def _sleep_instances_for_training_pool(self, instances: list[dict]) -> None:
        grouped: dict[tuple[PivotRL_Role, str], list[int]] = defaultdict(list)
        for instance in instances:
        ...
        async def _sleep_group(instance_role: PivotRL_Role, instance_model_name: str, instance_ids: list[int]) -> None:
            coordinator = self.coordinators[instance_role][instance_model_name]
            result = await self._await_elastic_coordinator_command(
                coordinator,
    ...
    async def enter_training_pool(self, instances: list[dict]) -> None:
        ...
        try:
            ...
            for instance in normalized:
                ...
                self.instances_engine_stats[role_name][model_name][instance_id] = None
            pivotrl_logger.info("Reserved trainer-pool elastic instances for training: %s.", normalized)
            entered = True
        finally:
    ...
    async def leave_training_pool(self, instances: list[dict]) -> None:
        ...
        pivotrl_logger.info("Released trainer-pool elastic instances from training: %s.", normalized)
    ...
    def _find_instances_to_scaled_down_for_other_roles(self, role_need_to_scale_up: dict):
        ...
        removable_budget: dict[tuple[PivotRL_Role, str], int] = {}
        min_awake_per_role = max(0, int(getattr(self.scaling_policy, "min_awake_per_role", 0)))
        ...
        async def _one(ref, key):
            ...
            try:
                ...
                pivotrl_logger.warning(
                    ...
                    "(If PIVOTRL_ELASTIC_RM_BACKLOG_DIAG=1, compare coordinator/router stages above.)",
                    op_label,
                    timeout_s,
                    key,
                )
    ...
    async def _sync_engine_status_from_coordinators(self):
        refs = []
        task_keys: list[tuple[PivotRL_Role, str]] = []
        for role_name, model_name in self.roles:
            coordinator = self.coordinators.get(role_name, {}).get(model_name)
            if coordinator is None:
                continue
    ...
    async def _sync_router_backlog_from_coordinators(self):
        ...
        grouped: dict[PivotRL_Role, list] = defaultdict(list)
        for (role_name, model_name), result in zip(task_keys, results):
            grouped[role_name].append(result)

        role_backlog: dict[PivotRL_Role, int] = {}
        role_summary: dict[PivotRL_Role, dict[str, int]] = {}
        for role_key, result_list in grouped.items():
            ...
            for result in result_list:
                if isinstance(result, Exception):
                    pivotrl_logger.warning("Failed to fetch router backlog for role=%s: %s", role_key, result)
    ...
    def _get_instance_kv_cache_usage(self, role_name: PivotRL_Role, model_name: str, instance_id: int) -> float:
    ...
    def _get_instance_running_waiting(
        self, role_name: PivotRL_Role, model_name: str, instance_id: int
    ) -> tuple[int, int]:
