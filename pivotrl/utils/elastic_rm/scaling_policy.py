...
class _DecisionCycleLogger:
    ...
    def flush(self) -> None:
        if not self._enabled or not self._lines:
            return
        cid = self._cycle_id
        pivotrl_logger.info("elastic_rm_policy ========== cycle %d BEGIN ==========", cid)
        for line in self._lines:
            pivotrl_logger.info("elastic_rm_policy cycle=%d | %s", cid, line)
        pivotrl_logger.info("elastic_rm_policy ========== cycle %d END ==========", cid)
...
class ScalingDecision:
    ...
    role_to_total_mu: dict[PivotRL_Role, float]
...
class ThroughputProfileLoader:
    ...
    @staticmethod
    def _resolve_new_schema_entry(payload: dict, signal: InstanceSignal, role_name: PivotRL_Role) -> dict | None:
...
class ScalingPolicy:
    def __init__(self, config: dict, policy_config: dict | None = None):
        ...
        pivotrl_logger.propagate = False
        pivotrl_logger.addHandler(FileOnlyHandler(self.config.pivotrl.logging_path, self.log_prefix))
    ...
    @staticmethod
    def _router_backlog_positive(router_backlog_by_role: dict[PivotRL_Role, Any] | None) -> bool:
    ...
    def _policy_log(self, event: str, **kwargs: Any) -> None:
        ...
        pivotrl_logger.info("elastic_rm_policy %s | %s", event, " ".join(parts))
    ...
    def _awake_bundle_keys(
        ...
        exclude: set[tuple[PivotRL_Role, str, int]] | None = None,
    ) -> set[tuple[str, int]]:
    ...
    def _pick_migration_targets_for_conflicts(
        ...
        *,
        ...
        instance_mu: dict[tuple[PivotRL_Role, str, int], float],
    ...
    @staticmethod
    def _group_by_role(signals: list[InstanceSignal]) -> dict[PivotRL_Role, list[InstanceSignal]]:
        """
        Group the signals by role.
        Args:
            signals: list[InstanceSignal]
        Returns:
            dict[PivotRL_Role, list[InstanceSignal]]:
    ...
    def _estimate_lambda(self, signals: list[InstanceSignal], role_to_total_mu: dict[PivotRL_Role, float]) -> float:
        """
    ...
    def _build_mu_maps(
        ...
        dict[tuple[PivotRL_Role, str, int], float],
        dict[PivotRL_Role, float],
        dict[tuple[PivotRL_Role, str, int], str],
    ]:
        """
        ...
        Args:
            signals: list[InstanceSignal]
        Returns:
            tuple[dict[tuple[PivotRL_Role, str, int], float], dict[PivotRL_Role, float]]:
                - instance_mu: {(role, model_name, instance_id): mu}
                - role_total_mu: {role: total_mu}
        """
        instance_mu: dict[tuple[PivotRL_Role, str, int], float] = {}
        instance_mu_source: dict[tuple[PivotRL_Role, str, int], str] = {}
        role_total_mu: dict[PivotRL_Role, float] = {}
        for signal in signals:
    ...
    def _resolve_role_request_cap(self, role_name: PivotRL_Role, model_name: str) -> int:
        """Resolve per-instance request cap for backlog-based batch scale-up."""
        if role_name == PivotRL_Role.Rollout:
            routing_cfg = self._cfg_get(self._cfg_get(self.config, "pivotrl"), "routing_strategy")
        ...
        if role_name == PivotRL_Role.RewardModel:
    ...
    def _make_stepwise_decision(
        ...
        rollout_role = PivotRL_Role.Rollout
        rm_role = PivotRL_Role.RewardModel
        cy = self._cycle_log
    ...
    def decide(
        ...
        Args:
            ...
            router_backlog_by_role: Pending request counts at the router, keyed by
                ``PivotRL_Role``. Drives force-wake when a role has router backlog but no
        ...
        for role_name, role_signals in grouped.items():
            ...
            if pending_cnt > 0:
                ...
                continue
            other_role = PivotRL_Role.RewardModel if role_name == PivotRL_Role.Rollout else PivotRL_Role.Rollout
