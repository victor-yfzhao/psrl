...
Config lives under ``policy_config["itl_policy"]`` (see ``pivotrl.yaml``); select
...
from pivotrl.utils.elastic_rm.itl_scaling_policy import ITLScalingPolicy, _RouterWaitingLoad
...
class ITLHarmonicScalingPolicy(ITLScalingPolicy):
    ...
    def _system_throughput(
        ...
        # more capacity.
        if starved_role == PivotRL_Role.Rollout:
    ...
    def _combine_request_level_role_throughputs(
        ...
        rollout_waiting = self._normalize_router_waiting_load(
            backlog_map.get(PivotRL_Role.Rollout)
        )
        rm_waiting = self._normalize_router_waiting_load(
            backlog_map.get(PivotRL_Role.RewardModel)
        )
        ...
        if starved_role == PivotRL_Role.Rollout:
            ...
            return role_throughputs[PivotRL_Role.RewardModel] / divisor
        if starved_role == PivotRL_Role.RewardModel:
            divisor = w_rollout if w_rollout > 0.0 else 1.0
            return role_throughputs[PivotRL_Role.Rollout] / divisor
        return self._weighted_system_throughput(
            role_throughputs[PivotRL_Role.Rollout],
            role_throughputs[PivotRL_Role.RewardModel],
            w_rollout,
            w_rm,
        )
