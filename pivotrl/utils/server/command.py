...
class CommandExtension:
    ...
    async def exec_command(self, command: Command, timeout=None, blocking=True):
        ...
        if not blocking:
            pivotrl_logger.debug(f"Command {command.type} with ID {command_id} is non-blocking, returning command ID.")
            return command_id
        ...
        pivotrl_logger.debug(f"Command {command_id} completed with result: {result}")
    ...
    async def synchronize_command(self, command_id: int, timeout=None):
        ...
        success = await command_event.wait(timeout=timeout)
        if not success:
            pivotrl_logger.warning(f"Command event {command_id} timed out after {timeout} seconds.")
            return None
    ...
    def _complete_command(self, command_id: int, result: Any):
        ...
        pivotrl_logger.debug(f"Command ID {command_id} completed with result: {result}")
        if command_id in self._command_events:
