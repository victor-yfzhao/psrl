...
from pivotrl.tools.base import Tool
from pivotrl.tools.sandbox_fusion_tool import DEFAULT_TIMEOUT
    ...
    try:
        if not isinstance(test_cases, dict):
            try:
                ...
                pivotrl_logger.error(f"Failed to parse test_cases JSON: {e}")
        ...
        if test_cases is not None and "assert_case" in test_cases and isinstance(test_cases.get("assert_case"), list):
            ...
            pivotrl_logger.error("Invalid test_cases structure.")
        ...
        pivotrl_logger.error(f"Error during compute_score: {e}")
        traceback.print_exc()
        score = 0.0
...
async def check_correctness(
    ...
    """
    pivotrl_logger.info("Starting correctness check for generation.")

    if not in_outs or "inputs" not in in_outs or "outputs" not in in_outs:
        pivotrl_logger.warning("Invalid in_outs format provided.")
    ...
    if num_cases == 0:
        pivotrl_logger.warning("Empty inputs provided.")
        return [], []
    ...
    for i, task_result in enumerate(task_results):
        if task_result.error:
            pivotrl_logger.error(f"Test case {i} generated an error: {task_result.error}")
    ...
    pivotrl_logger.info(f"Correctness check finished. Results: {results}")
