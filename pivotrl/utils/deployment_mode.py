...
`resolve_deployment_mode` maps the unified `pivotrl.deployment.mode` config entry
...
(which lays out resource pools) and before `PivotRL_RayPPOTrainer.__init__` derives
its mode flags.
...
def resolve_deployment_mode(config) -> str:
    ...
    """
    deployment = config.pivotrl.deployment
    ...
    if mode is None:
        ...
        if deployment.elastic_rm.get("enable", False):
            mode = "elastic_rl"
        elif config.pivotrl.get("colocate", False):
    ...
    if mode not in VALID_DEPLOYMENT_MODES:
        raise ValueError(f"Invalid pivotrl.deployment.mode={mode!r}; expected one of {VALID_DEPLOYMENT_MODES}.")

    with open_dict(config):
        config.pivotrl.deployment.mode = mode
        elastic_rm = config.pivotrl.deployment.elastic_rm
...
def validate_trainer_sleep_optimizer_offload(config, mode: str | None = None) -> None:
    ...
    if bool(deployment.elastic_rm.get("enable_trainer_pool", False)):
        sleep_reasons.append("pivotrl.deployment.elastic_rm.enable_trainer_pool=True")
    if resolved_mode == "trainer_pool_only":
        sleep_reasons.append("pivotrl.deployment.mode=trainer_pool_only")
    if bool(config.pivotrl.get("colocate_validate_and_train", False)):
        sleep_reasons.append("pivotrl.colocate_validate_and_train=True")
