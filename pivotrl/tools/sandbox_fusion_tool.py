...
from pivotrl.tools.base import Tool, ToolOutput
...
class SandboxFusionTool(Tool):
        ...
        try:
            ...
            # Case 2: With test cases - run correctness check
                ...
                try:
                    ...
                    pivotrl_logger.error(f"Test case {case_index} generated an exception: {exc}")
                    traceback.print_exc()
            ...
            pivotrl_logger.error(error_msg)
    ...
    async def _process_single_case(
        ...
        api_response = None
        error_msg = None
        pivotrl_logger.debug(f"Processing test case {case_index + 1}.")
    ...
    """
        ...
        try:
            ...
            pivotrl_logger.error(f"Case {case_index + 1}: {error_msg}")
            traceback.print_exc()
        ...
        if error_msg:
            ...
            pivotrl_logger.error(f"Case {case_index}: API error occurred: {error_msg}")
            # Log code and input only on error for brevity
            generation_to_log = generation[:200] + "..." if len(generation) > 200 else generation
            pivotrl_logger.error(f"Case {case_index}: code: {generation_to_log}")
            pivotrl_logger.error(f"Case {case_index}: input: {stdin}")
        elif api_response:
            # --- Add debug logging ---
            pivotrl_logger.debug(f"Case {case_index}: API Response: {api_response}")
            ...
            if api_status == "SandboxError":
                ...
                # --- Add debug logging ---
                pivotrl_logger.debug(f"API returned Failed status. Response: {api_response}")
                pivotrl_logger.debug(f"Compile Result: {compile_result}")
                pivotrl_logger.debug(f"Run Result: {run_result}")
                ...
                # Run failed or timed out
                    ...
                    pivotrl_logger.warning("API status Failed but cannot determine specific error type (compile/run).")
                ...
                pivotrl_logger.warning(f"Unknown API status received: {api_status}")
            ...
            result_status = -1
            pivotrl_logger.error(f"Case {case_index}: Unknown API state (no response and no error message).")
    ...
    async def call_sandbox_api(
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                async with aiohttp.ClientSession(timeout=timeout) as session:
                    async with session.post(
                        ...
                        if response.status == 504:
                            ...
                            pivotrl_logger.warning(last_error)
                            if attempt < MAX_RETRIES - 1:  # Don't sleep after the last attempt
                                delay = INITIAL_RETRY_DELAY * (attempt + 1)
                                pivotrl_logger.info(f"{log_prefix}Retrying after {delay} seconds...")
                        ...
                        pivotrl_logger.debug(f"{log_prefix}Sandbox API call successful on attempt {attempt + 1}")
                        response_json = await response.json()
                        return response_json, None

            except aiohttp.ClientResponseError as e:
                last_error = f"{log_prefix}API Response Error: {e.status} - {e.message}"
                pivotrl_logger.error(last_error)
                ...
                if attempt < MAX_RETRIES - 1:
                    delay = INITIAL_RETRY_DELAY * (attempt + 1)
                    pivotrl_logger.info(f"{log_prefix}Retrying after timeout in {delay} seconds...")
                    await asyncio.sleep(delay)
                    continue
                break
            except Exception as e:
        ...
        pivotrl_logger.error(f"{log_prefix}Sandbox API call failed. Last error: {last_error}")
