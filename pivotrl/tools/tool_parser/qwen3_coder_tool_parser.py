...
class Qwen3XMLToolParser(ToolParser):
    ...
    def extract_tool_calls_from_str(
        ...
        try:
            ...
            pivotrl_logger.exception("Failed to extract a Qwen3 XML tool call")
            return response_str, []
    ...
    def _convert_param_value(
        ...
        if parameter_type.startswith(("int", "uint", "long", "short", "unsigned")):
            try:
                ...
                pivotrl_logger.warning("Invalid integer %r for %s.%s", value, function_name, parameter_name)
                return value
        if parameter_type.startswith(("num", "float")):
            try:
                ...
                pivotrl_logger.warning("Invalid number %r for %s.%s", value, function_name, parameter_name)
                return value
