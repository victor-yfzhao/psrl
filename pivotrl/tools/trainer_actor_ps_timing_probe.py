...
from pivotrl.utils.common.worker_naming import ps_agent_name, train_client_name

REPO_ROOT = Path(__file__).resolve().parents[2]
CONFIG_DIR = REPO_ROOT / "pivotrl" / "trainer" / "config"
...
def _compose_config(args: argparse.Namespace, server_ip: str, server_port: int):
    ...
    defaults = [
        ...
        "++pivotrl.val_rollout_n=1",
        "pivotrl.tms.range=train",
        "pivotrl.tms.enable_nixl=True",
        "pivotrl.memory_logger.enable=False",
        f"pivotrl.ps_manager_ip={server_ip}",
        f"pivotrl.nixl.server_ip={server_ip}",
        f"pivotrl.nixl.server_port={server_port}",
        f"pivotrl.logging_path={logging_path}",
        f"pivotrl.nixl.weight_arena.actor_enabled={args.weight_arena}",
    ]
    if args.weight_arena:
        defaults.append(f"pivotrl.nixl.weight_arena.max_chunk_gb={args.weight_arena_max_chunk_gb}")
...
def main() -> None:
    ...
    from pivotrl.workers.ps import PSManager, PSStoragePlan, PSStorageWorker
    from pivotrl.workers.ps.ps_worker_group import (
        PSClassWithInitArgs,
        PSResourcePool,
        PSResourceSpec,
        PSWorkerGroup,
    )
    from pivotrl.workers.train.base_train_worker import TrainInterface
    from pivotrl.workers.train.fsdp_train_worker import PivotRL_FSDPTrainWorker
    ...
    try:
        ...
        actor_cls = RayClassWithInitArgs(
            cls=ray.remote(PivotRL_FSDPTrainWorker),
            config=config.train_actor_rollout_ref,
            role="actor",
            pivotrl_config=config.pivotrl,
        ...
        worker_env = {
            ...
            "PIVOTRL_TMS_ENABLE": "1",
            "PIVOTRL_EMPTY_INIT_NO_RANDOM_WEIGHTS": "1",
        }
