...
from pivotrl.utils.ray import add_busy_polling_lock
from pivotrl.workers.ps.ps_worker_group import PSWorkerGroup
from pivotrl.workers.ps.request_status_tracker import RequestStatusTracker
from pivotrl.workers.ps.staleness_controller import (
    BufferStatus,
    EntryInfo,
    StalenessInventory,
)
...
class PSManager(RequestStatusTracker):
    def __init__(
        ...
        Args:
            pivotrl_config (DictConfig): Configuration object containing parameters such as
                rollout_n, staleness buffer entries, staleness limit, etc.
        """
        RequestStatusTracker.__init__(self, pivotrl_config)

        if self.pivotrl_config.redundant_rollout.enable:
            self.rollout_n = self.pivotrl_config.redundant_rollout.redundant_rollout_n
            self.alg_rollout_n = self.pivotrl_config.redundant_rollout.alg_rollout_n
        else:
            self.rollout_n = self.pivotrl_config.rollout_n
        ...
        if self.pivotrl_config.redundant_rollout.enable:
            entries_per_buffer = self.pivotrl_config.redundant_rollout.redundant_global_batch_size
            ready_entries_per_buffer = self.pivotrl_config.redundant_rollout.alg_global_batch_size
        else:
            entries_per_buffer = self.pivotrl_config.staleness_buffer_entries
        ...
        self.staleness_inventory = StalenessInventory(
            ...
            staleness=self.pivotrl_config.staleness,
        ...
        """
        pivotrl_logger.info(f"PSManager initialized on {get_worker_info()}.")
    ...
    def get_max_reserve_num(self, model_version, is_validate: bool = False) -> int:
        ...
        """
        max_staleness_buffer_id = model_version + self.pivotrl_config.staleness if not is_validate else None
    ...
    def can_reserve_request(
        ...
        # pivotrl_logger.info(f"Checking if request {request_idx} can be reserved for model versions: {model_versions}")
            ...
            if staleness_inventory.can_reserve_data_without_new_reserve_entry(entry_info, model_version):
                ...
                if is_validate:
                    ...
                    max_pending_buffer_id = staleness_inventory.get_max_pending_buffer_id(
                        model_version + self.pivotrl_config.staleness
                    )
    ...
    def abort_requests(
        ...
        for prompt_id, abort_request_idxs in prompt_id_to_abort_request_idxs.items():
            ...
            if abort_group and len(rest_requests_of_entry) < self.alg_rollout_n:
                ...
                pivotrl_logger.info(f"Abort entire entry: (buffer {buffer_id}, entry {entry_id}) for prompt {prompt_id}")
            else:
        ...
        pivotrl_logger.debug(f"Abort requests done: {abort_request_ids=}, {clear_entries=}")
    ...
    def _abort_after_buffer_ready(self, buffer_id: int) -> set[int]:
        ...
        if buffer_id >= self.pivotrl_config.staleness:
            ...
            self.check_abort_versions.add(buffer_id - self.pivotrl_config.staleness)
        ...
        while len(self.check_abort_versions) > 0:
            ...
            buffer_range = set(
                range(
                    ...
                    version_to_abort + self.pivotrl_config.staleness + 1,
                )
            )
        ...
        for abort_version in curr_abort_versions:
            ...
            pivotrl_logger.info(f"Requests of version {abort_version} to abort: {requests_of_abort_version}")
        ...
        for buffer_id in ready_buffer_ids:
            if self.staleness_inventory.buffers[buffer_id].get_reserve_entry_num() == 0:
                if curr_ps_model_version > buffer_id:
                    ...
                    pivotrl_logger.info(f"Marking buffer {buffer_id} ready for deletion after aborting requests.")
    ...
    def get_ps_model_version(self, debug_info: str = None) -> int:
        ...
        pivotrl_logger.debug(f"Getting PS model version from model store: {self.model_store}, debug info: {debug_info}.")
    ...
    def init_nixl_server(self, expected_agents: int):
        ...
        self.nixl_meta_server = NIXLMetaServer("NIXLMetaServer", self.pivotrl_config.nixl)
    ...
    def nixl_protocol(self):
        ...
        pivotrl_logger.info("nixl server protocol step 2: make unified sharding")
        self.nixl_meta_server.make_unified_sharding()
        pivotrl_logger.info("nixl server protocol step 3: notify all client shardings")
        self.nixl_meta_server.notify_all_client_shardings()
        pivotrl_logger.info(f"nixl server protocol step 4: waiting for {self.expected_agents} agents to send infos")
        self.nixl_meta_server.wait_for_client_infos(self.expected_agents)
        pivotrl_logger.info("nixl server protocol step 5: make comm plan")
        self.nixl_meta_server.make_comm_plan()
        pivotrl_logger.info("nixl server protocol step 6: notify all client infos and the global comm plan")
        ...
        pivotrl_logger.info("nixl server protocol step 8: notify all client temp mappings")
        self.nixl_meta_server.notify_all_client_temp_mappings()
        pivotrl_logger.info("nixl server protocol done.")
    ...
    def push_model_state_dict_cpu(self, version_tag: int, model_state_dict: Mapping[str, Tensor | DTensor] | None):
        ...
        """
        assert self.pivotrl_config.ps_mode == "cpu", "push_model_state_dict_cpu should only be used in 'cpu' mode."
    ...
    def push_model_state_dict_cpu_ref_list(self, version_tag: int, model_state_dict_ref_list: list[ray.ObjectRef]):
        ...
        """
        assert self.pivotrl_config.ps_mode == "cpu_ref", (
    ...
    def pull_model_state_dict_cpu(self, rollout_instance_id: int) -> Mapping[str, Tensor | DTensor] | None:
        ...
        """
        assert self.pivotrl_config.ps_mode == "cpu", "pull_model_state_dict_cpu should only be used in 'cpu' mode."
    ...
    def pull_model_state_dict_cpu_ref(self, rollout_instance_id: int) -> ray.ObjectRef:
        ...
        """
        assert self.pivotrl_config.ps_mode == "cpu_ref", "get_model_state_dict_ref should only be used in 'cpu_ref' mode."
        assert self.model_store is not None, "Model instance is not initialized."
