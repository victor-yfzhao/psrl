...
from pivotrl.utils.common.worker_naming import ps_agent_name, ps_client_pull_name, ps_client_push_name
from pivotrl.utils.converter import create_parameter_mapping
from pivotrl.utils.converter.hf_converter import convert_hf_inplace, maybe_convert_to_smaller_parts
from pivotrl.utils.converter.model_dtypes import fix_meta_model_dtypes
from pivotrl.utils.logger import get_ps_logger, get_worker_info, setup_ps_logger
...
class PSStorageWorker:
    ...
    def _nixl_protocol_phase1(self):
        ...
        pivotrl_logger.info("nixl client protocol step 0: convert_hf_inplace")
    ...
    def nixl_protocol(self):
        pivotrl_logger.info("nixl protocol start with two phases.")
    ...
    def init_model(self):
        ...
        if self.pivotrl_config.ps_mode in ("nixl_cpu", "nixl_gpu"):
            ...
            raise ValueError(f"Invalid PS mode: {self.pivotrl_config.ps_mode}")
        ...
        if self._tied_weights_alias_map:
            pivotrl_logger.info(f"init_model: detected tied-weight aliases: {self._tied_weights_alias_map}")
        ...
        pivotrl_logger.info(f"init_model (meta-only) done on {get_worker_info()}.")
    ...
    def _fix_meta_model_dtypes(meta_model: torch.nn.Module) -> None:
        ...
        if fixed_count:
            pivotrl_logger.info("_fix_meta_model_dtypes: corrected %s tensor(s) to float32.", fixed_count)
    ...
    def _build_tied_weights_alias_map(
        ...
        pivotrl_logger.info(f"_build_tied_weights_alias_map: '{alias}' (alias) <- '{canonical}' (canonical)")
    ...
    def preload_checkpoint_to_cpu(self) -> None:
        ...
        pivotrl_logger.info(f"[preload_checkpoint_to_cpu] Reading {len(shard_files)} shard file(s) under {local_path}.")

        cache: dict[str, torch.Tensor] = {}

        for shard_file in shard_files:
            pivotrl_logger.debug(f"[preload_checkpoint_to_cpu] Opening shard {shard_file}.")
