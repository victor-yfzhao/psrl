...
from pivotrl.utils.logger import DualOutputHandler, deprecated, get_ps_logger
...
class RequestStatusTracker:
    ...
    def __init__(self, pivotrl_config: DictConfig):
        self.pivotrl_config = pivotrl_config
        self._request_id_to_status: dict[int, PivotRL_RequestStatus] = {}  # Maps request ID to their statuses
        self._request_infos = {}  # Maps request IDs to EntryInfo objects
        # Maps statuses to sets of request IDs for quick access
        self._status_to_request_ids = {status: set() for status in PivotRL_RequestStatus}
        ...
        pivotrl_logger.info("Initialized RequestStatusTracker.")
    ...
    def update_request_status(
        ...
        status: list[PivotRL_RequestStatus] | PivotRL_RequestStatus,
        ...
        Args:
            ...
            status (Union[List[PivotRL_RequestStatus], PivotRL_RequestStatus]): The new status(es) to set
        ...
        for s in status:
            if s not in PivotRL_RequestStatus:
        ...
        for i, req_id in enumerate(request_id):
            ...
            if req_id in self._request_id_to_status:
                ...
                if old_status != new_status:
                    ...
                    # pivotrl_logger.info(f"Changed status of request {req_id}: {old_status.name} -> {new_status.name}")
    ...
    def get_request_status(self, request_id: list[int] | int):
        ...
        Returns:
            Union[List[PivotRL_RequestStatus], PivotRL_RequestStatus]: The current status(es) of the request(s)
    ...
    def remove_train_ready_request(self, request_id: list[int] | int):
        ...
        """
        pivotrl_logger.debug(f"Removing train ready request {request_id} from status tracker after reward completion")
        if not isinstance(request_id, list):
            request_id = [request_id]
        ...
        for req_id in request_id:
            ...
            assert self._request_id_to_status[req_id] == PivotRL_RequestStatus.COMPLETED, (
    ...
    def get_requests_by_status(self, status: PivotRL_RequestStatus) -> set:
        """
        Get all requests that have a specific status.

        Args:
            status (PivotRL_RequestStatus): The status to filter requests by.
    ...
    def add_request(
        ...
        rollout_instance_id: list[int] | int = -1,
        model_version: list[int] | int = -1,
        status: (list[PivotRL_RequestStatus] | PivotRL_RequestStatus) = PivotRL_RequestStatus.PENDING,
        ...
        Args:
            ...
            status (Union[List[PivotRL_RequestStatus], PivotRL_RequestStatus], optional):
                The initial status(es) of the request(s). Defaults to PivotRL_RequestStatus.PENDING.
    ...
    def _abort_requests(self, request_ids: list[int] | int, blocking: bool = False):
        ...
        if filtered_request_ids:
            pivotrl_logger.info(f"Added requests {filtered_request_ids} to abort set")
        ...
        if abort_requests_for_rollout:
            ...
            pivotrl_logger.debug(f"Abort command sent to rollout coordinator for requests: {abort_requests_for_rollout}")

        # Abort requests in reward stage (REWARD_RUNNING)
        if abort_requests_for_reward:
            pivotrl_logger.debug(f"Aborting requests in reward stages: {abort_requests_for_reward}")
            futures.append(
            ...
            pivotrl_logger.debug(f"Abort command sent to reward manager for requests: {abort_requests_for_reward}")
    ...
    def remove_request(self, request_id: list[int] | int):
        ...
        """
        pivotrl_logger.debug(f"Removing request {request_id} from status tracker")
        if not isinstance(request_id, list):
            request_id = [request_id]
