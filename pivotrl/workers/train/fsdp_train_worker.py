...
from pivotrl.utils.common.patch_utils import apply_tms_patch
from pivotrl.utils.common.utils import lazy_import_to_globals
from pivotrl.utils.common.worker_naming import train_client_name
...
from pivotrl.utils.optimizer_state import assert_optimizer_state_offloaded
from pivotrl.utils.ray import exclusive_push_model_context
from pivotrl.workers.train import PivotRL_BaseTrainWorker, TrainInterface
...
class PivotRL_FSDPTrainWorker(ActorRolloutRefWorker, PivotRL_BaseTrainWorker):
    def __init__(
        self,
        config: DictConfig,
        role: str,
        ...
        PivotRL_BaseTrainWorker.__init__(
            self,
            self.rank,
            self.world_size,
            pivotrl_config,
        ...
        if self.pivotrl_config.tms.range in ["train", "all"]:
        ...
        if torch.cuda.is_available() and self.pivotrl_config.memory_logger.enable:
            self.memory_logger = MemoryLogger(
                self.pivotrl_config.logging_path,
                f"{self.log_prefix}_Mem",
                interval_seconds=self.pivotrl_config.memory_logger.interval_seconds,
            )
    ...
    def _actor_weight_arena_config(self, role: str):
        if role != "actor":
            return None
        arena_config = self.pivotrl_config.nixl.get("weight_arena", None)
        if arena_config is None or not arena_config.get("actor_enabled", False):
            return None
        if self.pivotrl_config.ps_mode not in ("nixl_cpu", "nixl_gpu"):
            raise RuntimeError(
                "pivotrl.nixl.weight_arena.actor_enabled requires pivotrl.ps_mode to be nixl_cpu or nixl_gpu."
            )
        if self.pivotrl_config.tms.range not in ("train", "all"):
            raise RuntimeError(
                "pivotrl.nixl.weight_arena.actor_enabled requires pivotrl.tms.range to be train or all."
            )
    ...
    def _materialize_fsdp2_model_before_load(self, actor_module_fsdp: torch.nn.Module, role: str):
        ...
        if materialization != "direct":
            raise RuntimeError(
                "pivotrl.nixl.weight_arena.actor_materialization must be 'direct' or 'repack', "
                f"got {materialization!r}."
            )

        from pivotrl.utils.weight_arena import gb_to_bytes, materialize_fsdp2_model_weights_in_arena
    ...
    def _prepare_fsdp2_model_before_shard(
        ...
        from pivotrl.utils.weight_arena import prepare_module_for_direct_fsdp2_weight_arena
    ...
    def _post_fsdp2_model_load(self, actor_module_fsdp: torch.nn.Module, role: str) -> None:
        ...
        from pivotrl.utils.weight_arena import gb_to_bytes, pack_fsdp2_model_weights_in_fresh_tms_pool
    ...
    def init_nixl_client(self):
        ...
        self.nixl_storage_client = NIXLStorageClient(
            ...
            nixl_config=self.pivotrl_config.nixl,
            nixl_interface=self.nixl_interface,
            # client_group_id=self.get_replica_id()
            logging_path=self.pivotrl_config.logging_path,
        )
        pivotrl_logger.info(f"NIXL client initialized on port {self.nixl_storage_client.client_port}.")
    ...
    def nixl_protocol(self, mode: str = "full"):
        ...
        pivotrl_logger.info("nixl client protocol step 1: connect_to_server")
        self.nixl_storage_client.connect_to_server()
        pivotrl_logger.info("nixl client protocol step 2: send_local_sharding")
        self.nixl_storage_client.send_local_sharding(self.local_sharding_dict)
        pivotrl_logger.info("nixl client protocol step 3: wait_for_server_sharding")
        unified_sharding_dict = self.nixl_storage_client.wait_for_server_sharding()
        pivotrl_logger.info("nixl client protocol step 4: register_local_tensors")
        self.nixl_storage_client.register_local_tensors(
            self.unified_state_dict, unified_sharding_dict, meta_only=meta_only
        )
        pivotrl_logger.info("nixl client protocol step 5: send_local_info")
        self.nixl_storage_client.send_local_info()
        pivotrl_logger.info("nixl client protocol step 6: wait_for_server_info")
        self.nixl_storage_client.wait_for_server_info()
        pivotrl_logger.info("nixl client protocol step 7: send_local_temp_mapping")
        self.nixl_storage_client.send_local_temp_mapping()
        pivotrl_logger.info("nixl client protocol step 8: wait_for_server_temp_mappings")
        self.nixl_storage_client.wait_for_server_temp_mappings()
        pivotrl_logger.info("nixl client protocol done.")
    ...
    def clear_fsdp2_grads(self):
        ...
        from pivotrl.utils.weight_arena import get_fsdp_param_groups
        ...
        if dirty_count > 0:
            ...
            pivotrl_logger.debug(f"[clear_fsdp2_grads] No stale grads found on rank {self.rank}.")
        return dirty_count
    ...
    def nixl_sleep(self, mode: str = "full"):
        ...
        self._pivotrl_full_sleep_active = full_sleep
        if mode != "meta":
            if self.nixl_storage_client.local_client_info is None:
                pivotrl_logger.warning("Skip NIXL deregistration because local tensors are not registered.")
            else:
    ...
    def nixl_wake_up(self):
        ...
        if getattr(self, "_pivotrl_full_sleep_active", False):
            optimizer_summary = assert_optimizer_state_offloaded(
                self.actor_optimizer,
                stage="after_wake",
                rank=self.rank,
        ...
        self._pivotrl_full_sleep_active = False
        return result
    ...
    def ray_push_model(self) -> None:
        ...
        pivotrl_logger.info("Gathering the full state dict on the CPU of the representative rank.")
        ...
        if self.is_train_representative_rank:
            ...
            pivotrl_logger.info("Push the model via CPU on the representative rank (async).")
            if self.pivotrl_config.ps_mode == "cpu":
                ...
                raise NotImplementedError(
                    f"PivotRL TrainWorker does not support PS mode '{self.pivotrl_config.ps_mode}' yet."
                )
        else:
    ...
    def init_model(self, init_mode: str = "full"):
        ...
        with log_dual_events("Initialize model", pivotrl_logger, event_type=EventType.INIT):
            ...
            skip_random_init = skip_load_weight and os.getenv("PIVOTRL_EMPTY_INIT_NO_RANDOM_WEIGHTS", "0") == "1"
            if skip_random_init:
                try:
                ...
                pivotrl_logger.info("Skipping random weight initialization for empty FSDP model initialization.")
    ...
    def update_actor(self, data: DataProto):
        with log_dual_events("Train actor", pivotrl_logger, event_type=EventType.TRAIN):
        ...
        with context_manager:
            with log_dual_events("Push model", pivotrl_logger, event_type=EventType.PUSH):
                PivotRL_BaseTrainWorker.push_model(self)
        return output
