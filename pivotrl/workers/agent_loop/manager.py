...
class PivotRL_AgentLoopManager:
    def __init__(
        ...
        if self.config.pivotrl.redundant_rollout.enable:
            self.entries_per_buffer = self.config.pivotrl.redundant_rollout.redundant_global_batch_size
            self.ready_entries_per_buffer = self.config.pivotrl.redundant_rollout.alg_global_batch_size
        else:
            self.entries_per_buffer = self.config.pivotrl.staleness_buffer_entries
            self.ready_entries_per_buffer = self.config.pivotrl.staleness_buffer_entries
        ...
        if self.config.pivotrl.server_rollout.enable:
    ...
    def _post_process(self, inputs: DataProto) -> DataProto:
        ...
        if "raw_prompt_ids" not in inputs.non_tensor_batch:
            ...
            ## pivotrl_logger.info("Remove left padding from input ids to get raw_prompt_ids")
        else:
            raw_prompt_ids = inputs.non_tensor_batch["raw_prompt_ids"]

        ## pivotrl_logger.info("Left pad prompt ids begin")
        ...
        ## pivotrl_logger.info("Right pad response ids begin")
        ...
        ## pivotrl_logger.info("Right pad response masks begin")
        outputs = self.tokenizer.pad(
        ...
        ## pivotrl_logger.info("Multiply response mask and response attention mask begin")
        response_mask = response_mask * response_attention_mask
        ## pivotrl_logger.info("Concat prompt attention mask and response attention mask begin")
        attention_mask = torch.cat([prompt_attention_mask, response_attention_mask], dim=1)
        ## pivotrl_logger.info("Concat prompt ids and response ids begin")
        ...
        if False:
            ...
            ## pivotrl_logger.info("Compute position ids with attention mask begin")
        ...
        if self.config.pivotrl.log_prob.enable_rollout_engine_log_prob:
            ## pivotrl_logger.info("Rollout log probs processing begin")
        ...
        if not is_validate and not self.config.reward_models_config.launch_reward_fn_async:
            ## pivotrl_logger.info("Reward processing begin")
        ...
        ## pivotrl_logger.info("Return data proto")
    ...
    async def _train_dispatch_data(self):
        ...
        while not self.stop_train_dispatch_task:
            ...
            if data is None:
                pivotrl_logger.info("Received END signal, stopping agent loop manager train dispatch task.")
                self.stop_train_dispatch_task = True
                continue
            ...
            if expected_ps_version > self.curr_ps_version_tag:
                ...
                pivotrl_logger.info(f"ps model version updated to {self.curr_ps_version_tag}, continue to dispatch")
        ...
        pivotrl_logger.info("Agent loop manager train dispatch task stopped.")
    ...
    async def _val_dispatch_data(self):
        ...
        while not self.stop_val_dispatch_task:
            ...
            # Receive END signal to stop processing data queue
            if data is None:
                pivotrl_logger.info("Received END signal, stopping agent loop manager validation dispatch task.")
                self.stop_val_dispatch_task = True
                continue
        ...
        pivotrl_logger.info("Agent loop manager validation dispatch task stopped.")
        ...
        if self.running_loop and not self.stop_train_dispatch_task:
            ...
            if data is None:
                if not self.train_data_queue.empty():
                    ...
                    data.non_tensor_batch["version_tag"] = np.array([-1] * batch_size, dtype=int)
                    pivotrl_logger.info(f"Retry {batch_size} requests (the data is provided by the data queue)")
                ...
                data.non_tensor_batch["version_tag"] = np.array([-1] * batch_size, dtype=int)
                pivotrl_logger.info(f"Retry {batch_size} requests (the data is provided by the caller)")
                await self._inner_dispatch_data(data)
        else:
            pivotrl_logger.warning("Busy loop of the agent loop manager has stopped, the retry operation will be skipped")
    ...
    def _get_expected_ps_version(self):
        ...
        """
        if self.config.pivotrl.redundant_rollout.enable:
            buffer_size = self.config.pivotrl.redundant_rollout.redundant_global_batch_size * self.rollout_n
        else:
            buffer_size = self.config.pivotrl.staleness_buffer_entries * self.rollout_n
    ...
    async def _collect_results(self, is_validate: bool):
        ...
        while not self.stop_collect_task:
            ...
            await asyncio.sleep(0)
        pivotrl_logger.info(f"Stop collecting {'validation' if is_validate else 'training'} results")
    ...
    async def occupy_requests(self, request_data: DataProto):
        ...
        """
        # pivotrl_logger.info(f"Occupying requests {request_data.non_tensor_batch['uid']}")
            ...
            if rollout_n > 1:
                ...
                for sample_id in unique_sample_ids:
                    if len(self.rollout_request_tracker[sample_id]) >= alg_rollout_n:
                        ...
                        pivotrl_logger.debug(f"Stored child IDs: {stored_child_ids}, Abort child IDs: {abort_child_ids}")
                        ...
                        if abort_child_ids:
                            ...
                            pivotrl_logger.info(f"Aborting child requests {abort_child_ids} for sample {sample_id}.")
    ...
    def maybe_add_buffer(self, buffer_id, data_buffer, is_validate: bool = False) -> bool:
        ...
        if is_validate:
            ...
            pivotrl_logger.debug(f"VALIDATION buffer {buffer_id} is added to val_data_buffers without post-processing.")
            return True
        ...
        if add_buffer:
            ...
            pivotrl_logger.debug(f"TRAINING buffer {buffer_id} is added to train_data_buffers after post-processing.")
        return add_buffer
    ...
    async def handle_ready_buffer(self, buffer_id: int, is_validate: bool = False):
        ...
        if is_validate:
            ...
            if buffer_id in self._val_buffer_waiters:
                ...
                pivotrl_logger.warning(f"No waiters found for VALIDATION buffer {buffer_id} when trying to awake.")
            self._finish_validation_recovery(buffer_id)
        ...
        pivotrl_logger.info(f"Checking staleness and aborting requests for buffer {buffer_id}.")
    ...
    async def handle_waiting_buffer(self, buffer_id: int):
        ...
        """
        if self.config.pivotrl.proactive_filter_strategy.method is None:
            return
        elif self.config.pivotrl.proactive_filter_strategy.method == "retry":
            ...
            if gap <= self.config.pivotrl.proactive_filter_strategy.threshold:
                pivotrl_logger.info(
                ...
                for _ in range(aborted_entry_num):
                    await self._retry_data()
        elif self.config.pivotrl.proactive_filter_strategy.method == "truncate":
            raise NotImplementedError("Truncate strategy is not implemented yet.")
        else:
            raise ValueError(f"Invalid proactive filter strategy: {self.config.pivotrl.proactive_filter_strategy.method}")
    ...
    async def wait_for_training_batch(self, buffer_id: int) -> DataProto:
        ...
        if await self._try_prepare_training_batch(buffer_id):
            pivotrl_logger.info(f"TRAINING Buffer {buffer_id} is ready, returning immediately.")
        ...
        pivotrl_logger.info(f"TRAINING Buffer {buffer_id} is not ready, waiting for it to be ready.")
    ...
    async def wait_for_validation_batch(self, buffer_id: int) -> DataProto:
        ...
        if buffer_id in self.val_data_buffers:
            ...
            pivotrl_logger.info(f"VALIDATION buffer {buffer_id} is ready, returning immediately.")
            return self.consume_validate_buffer(buffer_id)

        pivotrl_logger.info(f"VALIDATION buffer {buffer_id} is not ready, waiting for it to be ready.")
        fut = asyncio.get_event_loop().create_future()
    ...
    def log_buffer(self, buffer_id: int, is_validate: bool = False):
        ...
        # Log statistics
        pivotrl_logger.info(f"{'VALIDATION' if is_validate else 'TRAINING'} buffer {buffer_id} version tag distribution:")
        for version_tag in sorted(version_tag_counts.keys()):
            ...
            pivotrl_logger.info(f"version_tag={version_tag}: count={count} ({percentage:.2f}%), staleness={staleness}")
