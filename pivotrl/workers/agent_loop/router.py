...
from pivotrl.utils.elastic_rm.diagnostics import log_elastic_rm_backlog_diag
from pivotrl.utils.elastic_rm.overhead import RequestMigrationOverheadTracker
from pivotrl.utils.logger import DualOutputHandler, EventType, deprecated, log_dual_events
from pivotrl.utils.ray import AsyncBusyPollingRayLock
from pivotrl.utils.rollout.reprefill import filter_rollout_request_ids
...
from pivotrl.workers.agent_loop.request_queue import (
...
from pivotrl.workers.agent_loop.route_strategy import (
    RouteStrategyBase,
    get_route_strategy_class,
)
from pivotrl.workers.agent_loop.transition_tracker import InstanceTransitionTracker
from pivotrl.workers.gen.stats_collector import EngineStats
...
class RolloutRouter:
    def _partial_rollout_trace_enabled(self) -> bool:
        return bool(OmegaConf.select(self.config, "pivotrl.partial_rollout.trace.enable", default=False))
    ...
    def __init__(
        ...
        """
        self.config = config
        self.staleness = self.config.pivotrl.staleness
        ...
        if self.config.pivotrl.redundant_rollout.enable:
            ...
            self.balanced_concurrent_seqs_per_instance = (
                self.config.pivotrl.staleness_buffer_entries * self.rollout_n // self.n_rollout_instances
            )
        ...
        if self.config.pivotrl.routing_strategy.enable_multi_priority_queue:
            self.requests_to_route = MultiPriorityRequestQueue(
                self.staleness,
                request_sort_indicator=RequestSortIndicator(self.config.pivotrl.routing_strategy.request_sort_indicator),
            )
        else:
        ...
        self._exclusive_rebalance_migration_queue_enabled = bool(
            OmegaConf.select(
                self.config,
                "pivotrl.deployment.elastic_rm.itl_policy.exclusive_rebalance_migration_queue",
                default=False,
            )
        )
        ...
        self.log_prefix = "RolloutRouter"
        pivotrl_logger.addHandler(DualOutputHandler(self.config.pivotrl.logging_path, self.log_prefix))
        pivotrl_logger.info("Initialized RolloutRouter")
    ...
    def init_route_strategy(self, **kwargs):
        ...
        strategy_kwargs = {
            "logging_interval_in_ms": self.config.pivotrl.routing_strategy.logging_interval_in_ms,
            "cost_model_path": self.config.pivotrl.routing_strategy.cost_model_path,
            "model_name": rollout_model_name,
            "instance_to_tp_pp": instance_to_tp_pp,
            "max_num_waiting_reqs_after_preemption": (
                self.config.pivotrl.routing_strategy.max_num_waiting_reqs_after_preemption
            ),
            "balanced_concurrent_seqs_per_instance": self.balanced_concurrent_seqs_per_instance,
            "max_concurrent_seqs_per_instance": (self.config.pivotrl.routing_strategy.max_concurrent_seqs_per_instance),
            "delta_throughput_threshold": self.config.pivotrl.routing_strategy.delta_throughput_threshold,
            "max_prompt_length": self.config.data.max_prompt_length,
            "request_budget": self.config.pivotrl.routing_strategy.request_budget,
            "snapshot_staleness_threshold_in_ms": self.config.pivotrl.routing_strategy.snapshot_staleness_threshold_in_ms,
            "logger": pivotrl_logger,
            **kwargs,
        }
        try:
            route_strategy_class = get_route_strategy_class(self.config.pivotrl.routing_strategy.method)
            self.route_strategy: RouteStrategyBase = route_strategy_class(n_instances, strategy_kwargs)
            pivotrl_logger.info(f"Initialized route strategy: {self.config.pivotrl.routing_strategy.method}")
        except Exception as e:
            pivotrl_logger.warning(f"Route strategy error: {e}")
            pivotrl_logger.warning("Falling back to 'round_robin' strategy")
            from pivotrl.workers.agent_loop.route_strategy import RoundRobinRouteStrategy
    ...
    async def update_instance_status(self, instance_to_engine_status: dict[int, EngineStats], **kwargs):
        ...
        for instance_id, engine_status in instance_to_engine_status.items():
            if self.route_strategy.is_staled(instance_id, engine_status):
                # pivotrl_logger.warning(f"Instance {instance_id} collected engine status is stale, skipping")
                continue
    ...
    async def update_currently_syncing_instances(self, instance_ids: list[int], ps_model_version: int):
        ...
        pivotrl_logger.info(f"Updated currently syncing instances: {instance_ids} to version {ps_model_version}")
    ...
    async def wait_instance_transition(self, transition_id: int) -> None:
        ...
        pivotrl_logger.info("Waiting for instance transition %d requests to resolve", transition_id)
        while self._instance_transitions.pending_request_ids(transition_id):
            await asyncio.sleep(0.01)
        pivotrl_logger.info("Instance transition %d requests resolved", transition_id)
    ...
    def _start_exclusive_rebalance(self, instance_to_uids: dict, migration_context: dict) -> None:
        if not getattr(self, "_exclusive_rebalance_migration_queue_enabled", False):
            return
        request_ids = {
            canonical_pivotrl_request_id(uid)
    ...
    def _finish_exclusive_rebalance_request(self, request_id: Any, *, reason: str) -> None:
        request_key = canonical_pivotrl_request_id(request_id)
        ...
        if not self._rebalance_pending_request_ids:
            pivotrl_logger.info("Exclusive rollout rebalance completed; resuming normal routing.")
    ...
    def _exclusive_rebalance_destination(self, request: DataProto) -> int | None:
        request_key = canonical_pivotrl_request_id(request.non_tensor_batch["uid"][0])
    ...
    def prepare_request_migrations(
        ...
        source_by_uid = {
            canonical_pivotrl_request_id(request_id): int(instance_id)
        ...
        for migration in request_migrations or []:
            ...
            logical_request_id = canonical_pivotrl_request_id(engine_request_id)
            try:
    ...
    async def _choose_new_rollout_instance(self, request: DataProto) -> int:
        ...
        # pivotrl_logger.info(f"Choosing new rollout instance for request {request.non_tensor_batch['uid'][0]}")
        request_id = request.non_tensor_batch["uid"][0]
        ...
        if self.config.pivotrl.fuse_rollout_with_validate:
        ...
        force_reroute = bool(request.non_tensor_batch.get("_pivotrl_force_reroute", False))
        if (
            "rollout_instance_id" in request.non_tensor_batch
            and not self.config.pivotrl.sync_and_mig_strategy.mig.enable
        ...
        enable_multi_instance_group = self.config.pivotrl.routing_strategy.enable_group_sampling_on_multi_instances
        ...
        async def _route_with_candidates(
            ...
            *,
            ...
            if self.config.pivotrl.routing_strategy.candidate_sort_indicator == "version":
                for candidate in route_candidates:
            ...
            elif self.config.pivotrl.routing_strategy.candidate_sort_indicator == "reserve_capability":
                ...
                raise ValueError(
                    f"Invalid candidate sort indicator: {self.config.pivotrl.routing_strategy.candidate_sort_indicator}"
                )
        ...
        if chosen_rollout_instance is not None:
            request.non_tensor_batch.pop("_pivotrl_force_reroute", None)
    ...
    def _consolidate_responses(
        ...
        for i in range(batch_size):
            ...
            if (
                self.config.pivotrl.log_prob.enable_rollout_engine_log_prob
                and hasattr(vllm_output.outputs[0], "logprobs")
                and vllm_output.outputs[0].logprobs is not None
            ):
                if self.config.pivotrl.partial_rollout.interrupt_as_prompt:
        ...
        if self.config.pivotrl.log_prob.enable_rollout_engine_log_prob:
    ...
    async def generate_async(
        ...
        if self.scheduler_task is None:
            ...
            pivotrl_logger.info("Started routing loop")

        request_id = request.non_tensor_batch["uid"][0]
        is_validate = request.meta_info.get("validate", False)
        update_status_success = await self.ps_manager_handle.update_request_status.remote(
            [request_id],
            PivotRL_RequestStatus.ROLLOUT_ROUTING,
            is_validate=is_validate,
        )
        ...
        self.requests_to_route.put(request)
        # pivotrl_logger.info(f"Adding request {request_id} to priority queue")
    ...
    def get_pending_request_count(self) -> int:
        ...
        t0 = time.monotonic()
        log_elastic_rm_backlog_diag(pivotrl_logger, "stage=RolloutRouter_enter")
    ...
    def get_pending_request_summary(self, top_t: int | None = None) -> dict[str, int]:
        ...
        t0 = time.monotonic()
        log_elastic_rm_backlog_diag(pivotrl_logger, "stage=RolloutRouter_summary_enter")
    ...
    def _candidate_evaluation_queue_priority(self, request: DataProto):
        indicator = RequestSortIndicator(
            self.config.pivotrl.routing_strategy.request_sort_indicator
        )
    ...
    async def _candidate_evaluation_request_row(
        ...
        *,
        ...
        if old_instance is not None:
            ...
            if (
                not self.config.pivotrl.sync_and_mig_strategy.mig.enable
    ...
    async def get_candidate_evaluation_snapshot(self, top_t: int | None = None) -> dict:
        ...
        for instance_id in range(self.n_rollout_instances):
            ...
            for request_key in request_ids:
                logical_request_id = canonical_pivotrl_request_id(request_key)
    ...
    async def pause_routing(self):
        ...
        while self._is_routing:
            await asyncio.sleep(self.config.pivotrl.routing_strategy.check_interval_in_ms / 1000)
        pivotrl_logger.info("Pausing routing")
    ...
    async def resume_routing(self):
        ...
        self._pause_routing = False
        self._wake_routing_loop()
        pivotrl_logger.info("Resuming routing")
    ...
    async def _single_priority_queue_routing_loop(self):
        ...
        """
        pivotrl_logger.info("Started single priority queue routing loop")
        while True:
            self._is_routing = False
            async with (
                ...
                while (
                    ...
                    # time_end = time.time()
                    # pivotrl_logger.info(f"Choosing rollout instance for request {request_id} to {new_instance_id} in {time_end - time_begin} seconds") # noqa: E501
            ...
            self._is_routing = False
            sleep_time = self.config.pivotrl.routing_strategy.check_interval_in_ms / 1000
    ...
    async def _multi_priority_queue_routing_loop(self):
        ...
        """
        pivotrl_logger.info("Started multi priority queue routing loop")
        while True:
    ...
    async def _route_single_request(self, request: DataProto, new_instance_id: int):
        ...
        update_status_success = await self.ps_manager_handle.update_request_status.remote(
            [request_id],
            PivotRL_RequestStatus.ROLLOUT_DISPATCHED,
        ...
        if update_status_success[0]:
            ...
            if (
                self.config.pivotrl.log_prob.enable_rollout_engine_log_prob
                and (
                    self.config.pivotrl.log_prob.get("update_reprefill_log_probs", False)
                    or self.config.pivotrl.partial_rollout.interrupt_as_prompt
                )
            ...
            # Generate response
            # pivotrl_logger.info(f"Generating response for request {request_id} on instance {new_instance_id}")
            ...
            if update_status == PivotRL_RequestStatus.ROLLOUT_INTERRUPTED_BY_SCHEDULER:
                pivotrl_logger.debug(
                ...
                if int(request_id) in self._force_reroute_request_ids:
                    consolidated_output.non_tensor_batch["_pivotrl_force_reroute"] = np.array([True], dtype=bool)
                    self._force_reroute_request_ids.discard(int(request_id))
                request_key = canonical_pivotrl_request_id(request_id)
                ...
                return
            elif update_status == PivotRL_RequestStatus.ROLLOUT_INTERRUPTED:
                pivotrl_logger.debug(
                    f"Request {request_id} on instance {new_instance_id} was interrupted "
                ...
                # No result to set since the request is not completed
                return
            elif update_status == PivotRL_RequestStatus.ROLLOUT_COMPLETED:
                ...
                self._planned_migration_destinations.pop(str(request_id), None)
                # pivotrl_logger.info(f"Request {request_id} on instance {new_instance_id} is aborted")
                result = None
                self._track_transition_request_resolved(request_id)
        else:
            # Means the request is aborted
            # pivotrl_logger.info(f"Request {request_id} is aborted")
            self._planned_migration_destinations.pop(str(request_id), None)
            result = None
    ...
    async def check_should_migrate(self) -> list[int]:
        ...
        """
        # pivotrl_logger.info("Checking which instances should be interrupted to migrate to others due to starvation")
        ...
        for starved_instance_id in filtered_instance_ids:
            for instance_id in range(self.rollout_wg_size):
                ...
                if self.config.pivotrl.sync_and_mig_strategy.mig.indicator == "request_num":
                ...
                elif self.config.pivotrl.sync_and_mig_strategy.mig.indicator == "throughput":
                ...
                elif self.config.pivotrl.sync_and_mig_strategy.mig.indicator == "kv_cache":
                    ...
                    raise ValueError(
                        f"Unknown migrate indicator: {self.config.pivotrl.sync_and_mig_strategy.mig.indicator}"
                    )

                if ratio > self.config.pivotrl.sync_and_mig_strategy.mig.threshold:
                    # pivotrl_logger.info(
        ...
        if len(candidate_migrate_instance_ids) > 0:
            ...
            if self.config.pivotrl.sync_and_mig_strategy.mig.stop_indicator == "request_num":
                request_num = instance_to_status[migrate_instance_id].get_waiting_and_running_queue_size()
                if request_num < self.config.pivotrl.sync_and_mig_strategy.mig.stop_threshold:
                    return []
            elif self.config.pivotrl.sync_and_mig_strategy.mig.stop_indicator == "throughput":
                throughput = instance_to_status[migrate_instance_id].get_generation_throughput()
                if throughput < self.config.pivotrl.sync_and_mig_strategy.mig.stop_threshold:
                    return []
            elif self.config.pivotrl.sync_and_mig_strategy.mig.stop_indicator == "kv_cache":
                kv_cache_utilization = instance_to_status[migrate_instance_id].get_kv_cache_utilization()
                if kv_cache_utilization < self.config.pivotrl.sync_and_mig_strategy.mig.stop_threshold:
                    return []
            else:
                raise ValueError(
                    f"Unknown stop indicator: {self.config.pivotrl.sync_and_mig_strategy.mig.stop_indicator}"
                )
    ...
    async def check_should_sync(self, instance_id: int, ps_model_version: int) -> bool:
        ...
        """
        # pivotrl_logger.info(f"Checking if instance {instance_id} should synchronize with PS")
        ...
        if len(filtered_request_ids) > 0 and self.config.pivotrl.sync_and_mig_strategy.sync.check_req_before_sync:
            return False

        # 3. Check indicator to determine whether to synchronize with PS
        if self.config.pivotrl.sync_and_mig_strategy.sync.indicator == "request_num":
            ...
            pivotrl_logger.debug(
                ...
                f"threshold: {self.config.pivotrl.sync_and_mig_strategy.sync.threshold}"
            )
            if request_num > self.config.pivotrl.sync_and_mig_strategy.sync.threshold:
                return False
        elif self.config.pivotrl.sync_and_mig_strategy.sync.indicator == "throughput":
            ...
            if throughput > self.config.pivotrl.sync_and_mig_strategy.sync.threshold:
                return False
        elif self.config.pivotrl.sync_and_mig_strategy.sync.indicator == "kv_cache":
            ...
            raise ValueError(f"Unknown sync indicator: {self.config.pivotrl.sync_and_mig_strategy.sync.indicator}")
