...
from pivotrl.utils.common.worker_naming import gen_client_name, ps_agent_name
from pivotrl.utils.converter import create_parameter_mapping
from pivotrl.utils.converter.param_sync import precision_sensitive_parameter_stats
...
class vLLMWorkerExtension:
    ...
    def preload_weights_to_cpu_cache(self, weights_path: str | None = None, load_format: str | None = None) -> int:
        ...
        try:
            ...
            from pivotrl.utils.node_shared_weight_cache import validate_reward_cache_config

            cache_mode = validate_reward_cache_config(arena_config)
            if cache_mode == "node_shared":
                if not hasattr(self.model_runner, "_pivotrl_weight_arena_handle"):
                    raise RuntimeError("Node-shared reward CPU cache requires an initialized reward weight arena.")
                from pivotrl.utils.node_shared_weight_cache import fingerprint_safetensors_checkpoint
                ...
                self._pivotrl_streaming_weight_source = {
                    "model_config": model_config,
                    "load_config": load_config,
                    "model_id": model_id,
                    "checkpoint_fingerprint": checkpoint_fingerprint,
                }
                self._pivotrl_reward_weight_cache_state = {
                    "ready": True,
            ...
            self._pivotrl_cpu_weight_cache = cache
            self._pivotrl_reward_weight_cache_state = {
                "ready": True,
                "source": "checkpoint",
                "checkpoint_tensor_count": len(cache),
            }
            pivotrl_logger.info("Preloaded %d reward model tensors to CPU cache.", len(cache))
            return len(cache)
    ...
    def _publish_node_shared_reward_arena(
        ...
        shared_cache = publish_or_map_weight_arena(
            arena_handle,
            cache_root=arena_config.get("reward_node_cache_dir", "/dev/shm/pivotrl-rm-weight-cache"),
            job_id=job_id,
    ...
    def close_node_shared_weight_cache(self) -> None:
        ...
        shared_cache = getattr(self, "_pivotrl_node_shared_arena_cache", None)
        if shared_cache is None:
            return
        if hasattr(self, "_pivotrl_weight_arena_cpu_cache"):
            del self._pivotrl_weight_arena_cpu_cache
        gc.collect()
        shared_cache.close()
        del self._pivotrl_node_shared_arena_cache
    ...
    def load_weights_from_cpu_cache(self, blocking: bool = True):
        ...
        arena_handle = getattr(self.model_runner, "_pivotrl_weight_arena_handle", None)
        arena_cache = getattr(self, "_pivotrl_weight_arena_cpu_cache", None)
        try:
            model = self.model_runner.get_model()
            cache_state = getattr(self, "_pivotrl_reward_weight_cache_state", None)
            ...
            if arena_cache is not None:
                ...
                arena_handle.assert_module_weights_in_arena(model)
                model_validation_elapsed_s += time.perf_counter() - validation_start
                loaded_params = {"__pivotrl_weight_arena_restore__"}
            else:
                streaming_source = getattr(self, "_pivotrl_streaming_weight_source", None)
                process_local_source = hasattr(self, "_pivotrl_cpu_weight_cache")
                ...
                def cached_weights_generator():
                    if streaming_source is not None:
                        ...
                        return
                    for name, tensor in self._pivotrl_cpu_weight_cache:
                        if isinstance(tensor, DTensor):
                            tensor = tensor.full_tensor()
                ...
                if node_shared_first_load:
                    ...
                    self._pivotrl_node_shared_arena_cache = shared_cache
                    self._pivotrl_weight_arena_cpu_cache = shared_cache.arenas
                    ...
                    arena_bytes = sum(
                        ...
                        for tensor in self._pivotrl_weight_arena_cpu_cache
                    )
                    self._pivotrl_reward_weight_cache_state = {
                        "ready": True,
                        "source": "node_shared_arena",
                        "arena_count": len(self._pivotrl_weight_arena_cpu_cache),
                    ...
                    del self._pivotrl_streaming_weight_source
                    gc.collect()
                ...
                if arena_handle is not None and not node_shared_first_load:
                    ...
                    if cache_mode == "node_shared":
                        ...
                        self._pivotrl_weight_arena_cpu_cache = snapshot_weight_arena_to_cpu(
                            arena_handle,
                    ...
                    if cache_mode == "node_shared":
                        self._pivotrl_reward_weight_cache_state.update(shared_cache.diagnostics())
                    cache_transitioned = True
                    release_start = time.perf_counter()
                    if streaming_source is not None:
                        del self._pivotrl_streaming_weight_source
                    else:
                        del self._pivotrl_cpu_weight_cache
                    gc.collect()
                    cache_release_elapsed_s = time.perf_counter() - release_start
        except Exception as e:
        ...
        self._pivotrl_worker_stage_timings = getattr(self, "_pivotrl_worker_stage_timings", {})
        self._pivotrl_worker_stage_timings["load_weights_from_cpu_cache"] = {
    ...
    def get_last_tms_sleep_wake_timing(self, operation: str) -> dict:
        ...
        return {
            ...
            "tp_rank": self.get_instance_local_tp_rank(),
            "pid": os.getpid(),
            "timing": getattr(self, f"_pivotrl_last_{operation}_timing", None),
        }

    def get_last_worker_stage_timing(self, stage: str) -> dict:
        """Return the latest timing for a worker-extension stage."""
        timings = getattr(self, "_pivotrl_worker_stage_timings", {})
        return {
            "hostname": socket.gethostname(),
            "rank": self.get_instance_local_rank(),
            "tp_rank": self.get_instance_local_tp_rank(),
    ...
    def get_weight_arena_info(self) -> dict:
        ...
        handle = getattr(self.model_runner, "_pivotrl_weight_arena_handle", None)
        arena_cache = getattr(self, "_pivotrl_weight_arena_cpu_cache", None)
        cache_state = getattr(self, "_pivotrl_reward_weight_cache_state", None)
        cache_info = None
        if arena_cache is not None:
    ...
    def init_nixl_client(
        ...
        self.pivotrl_instance_id = int(instance_id)
    ...
    def nixl_protocol(self, config: DictConfig, mode: str = "full"):
        ...
        # pivotrl_logger.info(f"unified_sharding_dict: {unified_sharding_dict}")
    ...
    def nixl_register_after_wake_up(self):
        ...
        elapsed_s = time.perf_counter() - total_start
        self._pivotrl_worker_stage_timings = getattr(self, "_pivotrl_worker_stage_timings", {})
        self._pivotrl_worker_stage_timings["nixl_register_after_wake_up"] = {
    ...
    def nixl_deregister(self):
        ...
        self._pivotrl_worker_stage_timings = getattr(self, "_pivotrl_worker_stage_timings", {})
        self._pivotrl_worker_stage_timings["nixl_deregister"] = {"elapsed_s": elapsed_s}
        pivotrl_logger.warning(
    ...
    def _capture_weight_fingerprint(
        ...
        *,
        ...
        record = fingerprint_log_record(
            ...
            rank=self.get_instance_local_rank(),
            instance_id=self.pivotrl_instance_id,
            tp_rank=self.get_instance_local_tp_rank(),
    ...
    def nixl_pull_model_core(
        ...
        elapsed_s = time.perf_counter() - total_start
        self._pivotrl_worker_stage_timings = getattr(self, "_pivotrl_worker_stage_timings", {})
        self._pivotrl_worker_stage_timings["nixl_pull_model_core"] = {
