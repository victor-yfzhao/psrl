...
from pivotrl.utils.logger import deprecated
...
from pivotrl.workers.gen import StatCollector

pivotrl_logger = logging.getLogger(__name__)
...
class PivotRL_vLLMRollout:
    def __init__(
        ...
        **kwargs,
        ...
        """
        Initialize PivotRL vLLM rollout with specified configuration.

        Args:
            model_path: Path to the model to load
            config: Configuration object containing vLLM and PivotRL settings
        ...
        weight_arena_config = pivotrl_config.nixl.get("weight_arena", {})
        ...
        if direct_arena:
            ...
            load_format = "pivotrl_arena_dummy"
        ...
        if not config.enforce_eager and cudagraph_capture_sizes:
            if isinstance(cudagraph_capture_sizes, ListConfig):
                ...
                pivotrl_logger.warning(f"cudagraph_capture_sizes must be a list, but got {cudagraph_capture_sizes}")
        ...
        if self.config.prometheus.enable:
            assert self.pivotrl_config.server_rollout.enable, (
        ...
        if config.use_pivotrl_scheduler:
            llm_kwargs["scheduler_cls"] = "pivotrl.workers.gen.rollout_scheduler.RolloutScheduler"
        if self.is_reward_model and self.rm_config is not None:
            ...
            max_num_waiting_reqs_after_preemption = rm_routing_strategy.get(
                ...
                pivotrl_config.routing_strategy.max_num_waiting_reqs_after_preemption,
            )
        else:
            max_num_waiting_reqs_after_preemption = pivotrl_config.routing_strategy.max_num_waiting_reqs_after_preemption
        llm_kwargs["additional_config"] = {
            ...
            * pivotrl_config.routing_strategy.max_estimated_concurrent_seqs_per_instance,
            "pivotrl_nixl_weight_arena": weight_arena_config,
            "pivotrl_role": "reward" if self.is_reward_model else "rollout",
            "instance_id": kwargs.get("instance_id", 0),
        }
        pivotrl_logger.info(
            "vLLM scheduler config: role=%s use_pivotrl_scheduler=%s scheduler_cls=%s "
            "max_num_waiting_reqs_after_preemption=%s",
            "reward" if self.is_reward_model else "rollout",
            config.use_pivotrl_scheduler,
        ...
        if self.pivotrl_config.server_rollout.enable:
            ...
            stat_loggers = None
            if not config.disable_log_stats and pivotrl_config.status_collection.enable:
                pivotrl_logger.info(f"Enable status collection for rollout instance {kwargs.get('instance_id', 0)}")
            ...
            pivotrl_logger.info(f"Initialize AsyncLLM for rollout instance {kwargs.get('instance_id', 0)}")
        ...
        if self.is_pooling_model:
            ...
            pivotrl_logger.info(f"pooling_params: {self.pooling_params}")
            print(f"pooling_params: {normalize=}, {use_activation=}, {task=}")
            pivotrl_logger.info(f"Initialized PoolingParams for pooling model")
        else:
            ...
            pivotrl_logger.info(f"kwargs: {kwargs}")
    ...
    def post_process_outputs(
        ...
        update_reprefill_log_probs = should_update_reprefill_log_probs(
            self.pivotrl_config.log_prob.get("update_reprefill_log_probs", False),
            is_reward_model=self.is_reward_model,
        ...
        for i, uid in enumerate(uid_list):
            ...
            try:
                ...
                pivotrl_logger.warning(f"Failed to get metrics for request {uid}: {e}")
                metrics_list.append({})
            if self.is_pooling_model:
            ...
            if (
                ...
                and self.pivotrl_config.log_prob.enable_rollout_engine_log_prob
                ...
                elif self.pivotrl_config.partial_rollout.interrupt_as_prompt:
                    response_lengths = non_tensor_batch.get("response_unpadded_len", [0] * batch_size)
                    if np.isscalar(response_lengths):
                        curr_response_len = int(response_lengths)
                    else:
        ...
        if not self.is_pooling_model:
            if self.pivotrl_config.log_prob.enable_rollout_engine_log_prob:
    ...
    async def interrupt_all_requests_async(self) -> int:
        ...
        if interrupted_request_num > 0:
            pivotrl_logger.debug(f"Interrupted {interrupted_request_num} requests via abort_all().")
        else:
            pivotrl_logger.debug("No requests to interrupt via abort_all().")
