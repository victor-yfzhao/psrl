...
class RolloutCoordinator(CommandExtension):
    ...
    def __init__(
        ...
        Coordinates and manages rollout instances for PivotRL.
        ...
        Args:
            config: Configuration object containing PivotRL settings
        ...
        self.enable_elastic_rm = self.config.pivotrl.deployment.elastic_rm.enable
        ...
        interrupt_config = self.config.pivotrl.deployment.get("disaggregated_rollout_interrupt", {})
        ...
        deployment_mode = str(self.config.pivotrl.deployment.get("mode", ""))
        if self._disaggregated_interrupt_enabled:
            if deployment_mode != "disaggregated":
                raise ValueError(
                    "disaggregated_rollout_interrupt requires pivotrl.deployment.mode=disaggregated"
                )
            ...
            if not self.config.pivotrl.partial_rollout.enable:
                raise ValueError(
                    "disaggregated_rollout_interrupt requires pivotrl.partial_rollout.enable=True "
        ...
        self._elastic_exec_role_name = None    # PivotRL_Role for this coordinator's role
    ...
    async def init_route_strategy(self, tag: str = "rollout"):
        ...
        pivotrl_logger.info(f"Max model lens on {tag} instances: {max_model_lens}")
    ...
    async def init_nixl_client(self):
        ...
        pivotrl_logger.info(f"Initialized NIXL client on all {self.gen_wg_size} instances.")
    ...
    async def initial_pull_from_ps(self, tag: str = "rollout") -> None:
        ...
        pivotrl_logger.info(f"Initial PS pull complete for {len(wg_list)} {tag!r} instance(s).")
    ...
    async def start_busy_loop(self):
        ...
        if self.config.pivotrl.status_collection.enable:
        ...
        if self.config.pivotrl.sync_and_mig_strategy.method == "greedy":
        ...
        elif self.config.pivotrl.sync_and_mig_strategy.method == "status_based":
            ...
            raise NotImplementedError(
                f"Sync strategy {self.config.pivotrl.sync_and_mig_strategy.method} is not supported"
            )
        # Check if rollout migration is enabled
        if self.config.pivotrl.sync_and_mig_strategy.mig.enable:
            assert self.config.pivotrl.status_collection.enable, (
                "Rollout migration is only supported when status collection is enabled"
            )
            assert self.config.pivotrl.partial_rollout.enable, (
    ...
    async def _disaggregated_rollout_interrupt_loop(self):
        ...
        while not self.stop_disaggregated_interrupt:
            ...
            try:
                ...
                pivotrl_logger.exception("[DISAGG_INTERRUPT] periodic rollout interruption failed")
    ...
    async def _command_handler_loop(self):
        ...
        while not self.stop_command_handler:
            ...
            if not self.command_queue.empty():
                ...
                if command_type == CommandType.ABORT:
                    ...
                    pivotrl_logger.info(f"Received ABORT command, interrupted {interrupted_request_num} requests")
                    ...
                    try:
                        ...
                        pivotrl_logger.exception("SLEEP command failed for instances %s", instance_ids)
                        if self.rollout_router is not None:
                            await self.rollout_router.resume_instances.remote(instance_ids)
                        self._complete_command(command_id, False)
                    else:
                        success = True
                        pivotrl_logger.info("SLEEP command for instances %s completed", instance_ids)
                    ...
                    try:
                        ...
                        pivotrl_logger.info("WAKE_UP command for instances %s completed", instance_ids)
        ...
        pivotrl_logger.info("Background command handler of rollout coordinator has finished.")
    ...
    async def _broadcast_status_to_router(self):
        ...
        while not self.stop_broadcast_status_to_router:
            ...
            await asyncio.sleep(self.config.pivotrl.status_collection.coordinator_sync_interval_in_ms / 1000)
    ...
    async def _greedy_sync_and_migrate_loop(self):
        ...
        """
        pivotrl_logger.info("Starting greedy model synchronization and rollout migration loop")

        while not self.stop_sync_and_migrate:
            # Sleep for a period of time
            await asyncio.sleep(self.config.pivotrl.sync_and_mig_strategy.check_interval_in_ms / 1000)
            ...
            for instance_id in range(self.rollout_wg_size):
                ...
                if self.config.pivotrl.sync_and_mig_strategy.sync.seamless_train_version >= self.ps_model_version:
            ...
            elif not have_syncing_instance and self.config.pivotrl.sync_and_mig_strategy.mig.enable:
                # No instance is syncing with PS, check if migration is needed
                await self.check_and_migrate()

        pivotrl_logger.info("Greedy model synchronization and rollout migration loop stopped.")
    ...
    async def _status_based_sync_and_migrate_loop(self):
        ...
        """
        pivotrl_logger.info("Starting status based model synchronization and rollout migration loop")
        ...
        while not self.stop_sync_and_migrate:
            ...
            for instance_id, engine_stats in self.instance_to_engine_status.items():
                ...
                if await self._is_routing():
                    # pivotrl_logger.info(f"Skipping synchronization with PS for instance
                ...
                if self.config.pivotrl.partial_rollout.enable:
        ...
        pivotrl_logger.info("Status based model synchronization and rollout migration loop stopped.")
    ...
    def set_elastic_executor(
        ...
        Args:
            ...
            role_name: PivotRL_Role for this coordinator's role (e.g. PivotRL_Role.Rollout).
    ...
    def set_ps_model_version(self, version: int):
        ...
        pivotrl_logger.info(f"Updated PS model version to {version}")
    ...
    def update_ready_buffer(self, ready_buffer: int):
        ...
        pivotrl_logger.info(f"Updated ready buffers to: {self.ready_buffers}")
    ...
    async def _sync_instances_directly(
        ...
        *,
        ...
        with log_dual_events(
            ...
            pivotrl_logger.info("Paused routing for synchronization transition %d", transition_id)
            try:
                await self.rollout_router.update_currently_syncing_instances.remote(
                    instance_ids,
                    target_ps_version,
                ...
                if wait_interrupted_partial_requests_loop_back and self.config.pivotrl.partial_rollout.enable:
                ...
                pivotrl_logger.info("Resumed routing after synchronization transition %d", transition_id)
        ...
        try:
            ...
            with log_dual_events(
                ...
                if in_place_sync_ids:
                    ...
                    pivotrl_logger.info("In-place sync fallback: no runnable instances to sleep/wake_up.")
            ...
            for instance_id in rollout_sync_ids:
                ...
                pivotrl_logger.info("sync_with_ps: released sync-lock for instance %d.", instance_id)
            try:
    ...
    async def check_should_sync(self, instance_id: int) -> bool:
        ...
        """
        assert self.config.pivotrl.sync_and_mig_strategy.method == "status_based", (
    ...
    async def check_and_migrate(self, wait_interrupted_partial_requests_loop_back: bool = True):
        ...
        """
        assert self.config.pivotrl.sync_and_mig_strategy.mig.enable, "Rollout migration is not enabled"
        assert self.config.pivotrl.status_collection.enable, (
            "Rollout migration is only supported when status collection is enabled"
        )
        # pivotrl_logger.info("Checking if any instance is starving and doing migration if necessary")
        ...
        if migrate_instance_ids:
            with log_dual_events(
                ...
                await self.rollout_router.pause_routing.remote()
                pivotrl_logger.info("Interrupted routing for migration transition %d", transition_id)
                try:
                    ...
                    await self.rollout_router.finish_instance_transition.remote(transition_id, resume=True)
                    await self.rollout_router.resume_routing.remote()
                    pivotrl_logger.info("Resumed routing after migration transition %d", transition_id)
