...
from pivotrl.utils.logger import FileOnlyHandler
...
class StatCollector(StatLoggerBase):
    ...
    def __init__(
        ...
        # Build logger
        if self.pivotrl_config.status_collection.dump_logging_to_file_level != "none":
            ...
            pivotrl_logger.addHandler(FileOnlyHandler(self.pivotrl_config.logging_path, self.log_prefix))
            pivotrl_logger.info(f"Initialized StatCollector for instance {self.instance_id}.")
    ...
    def record_model_version_update(self, model_version: int):
        ...
        if self.pivotrl_config.status_collection.dump_logging_to_file_level != "none":
            pivotrl_logger.info(f"Snapshot (model version {self.model_version}): {snapshot}")
    ...
    def record(
        ...
        if self.pivotrl_config.status_collection.dump_logging_to_file_level != "none":
            if (
                ...
                >= self.pivotrl_config.status_collection.dump_logging_to_file_interval_in_ms / 1000.0
            ):
                if self.pivotrl_config.status_collection.dump_logging_to_file_level == "prompt":
                    if iteration_stats and snapshot["iteration_stats"]["num_prompt_reqs"] > 0:
                        pivotrl_logger.info(f"Snapshot (model version {self.model_version}): {snapshot}")
                elif self.pivotrl_config.status_collection.dump_logging_to_file_level == "generation":
                    if (
                        iteration_stats
                        ...
                        pivotrl_logger.info(f"Snapshot (model version {self.model_version}): {snapshot}")
                elif self.pivotrl_config.status_collection.dump_logging_to_file_level == "all":
                    pivotrl_logger.info(f"Snapshot (model version {self.model_version}): {snapshot}")
                else:
                    raise ValueError(
                        f"Invalid dump logging to file level: "
                        f"{self.pivotrl_config.status_collection.dump_logging_to_file_level}"
                    )
        ...
        if (
            ...
            >= self.pivotrl_config.status_collection.engine_sync_interval_in_ms / 1000.0
        ):
            self.output_queue.put_nowait(
                EngineStats(
                    instance_id=self.instance_id,
