...
from pivotrl.utils.cost_model_path import resolve_cost_model_json_path
from pivotrl.utils.elastic_rm.candidate_routing import select_itl_candidate
...
pivotrl_logger = logging.getLogger("reward_model_router")
...
class RewardModelRouteStrategyBase(ABC):
    ...
    def __init__(self, n_instances: int, strategy_kwargs: dict | None = None):
        ...
        self.logger.setLevel(os.getenv("PIVOTRL_LOGGING_LEVEL", "WARN"))
...
class PivotRL_RewardModelRouter:
    ...
    def _get_elastic_rm_config(self) -> dict[str, Any]:
        try:
            cfg = self.config.pivotrl.deployment.elastic_rm
    ...
    def _init_route_strategy(self) -> RewardModelRouteStrategyBase:
        ...
        try:
            ...
            pivotrl_logger.info("Initialized reward-model route strategy: %s.", method)
            return strategy
        except Exception as exc:
            pivotrl_logger.warning("Reward-model route strategy error: %s.", exc)
            pivotrl_logger.warning("Falling back to reward-model round_robin strategy.")
    ...
    async def generate_async(self, request: DataProto) -> DataProto | None:
        ...
        if self.scheduler_task is None:
            ...
            pivotrl_logger.info("[router] Started reward routing loop")
    ...
    async def _route_single_request(
        ...
        if self._interrupt_routing:
            ...
            exclusive_pending_uids = {
                canonical_pivotrl_request_id(request_uid)
                for request_uid in request_uid_values
                if canonical_pivotrl_request_id(request_uid)
            ...
            if not exclusive_enqueued:
                for request_uid in request_uid_values:
                    if canonical_pivotrl_request_id(request_uid) in exclusive_pending_uids:
                        self._migration_overhead.discard(request_uid)
                self._enqueue_request(request_key, request)
        ...
        if result is None:
            pivotrl_logger.debug("Request %s interrupted or unavailable, requeueing original request.", request_uids)
            exclusive_pending_uids = {
                canonical_pivotrl_request_id(request_uid)
                for request_uid in request_uid_values
                if canonical_pivotrl_request_id(request_uid)
            ...
            for request_uid in request_uid_values:
                ...
                elif canonical_pivotrl_request_id(request_uid) in exclusive_pending_uids:
                    self._migration_overhead.discard(request_uid)
                else:
        ...
        try:
            ...
            interrupted = False
        if interrupted:
            pivotrl_logger.debug("Request %s interrupted, requeueing partial output for continuation.", request_uids)
            exclusive_pending_uids = {
                canonical_pivotrl_request_id(request_uid)
                for request_uid in request_uid_values
                if canonical_pivotrl_request_id(request_uid)
        ...
        if pivotrl_logger.isEnabledFor(logging.DEBUG):
            pivotrl_logger.debug("[router] Reward request %s finished on worker %d", request_uids, worker_idx)
        for request_uid in request_uid_values:
            self._planned_migration_destinations.pop(request_uid, None)
    ...
    def _finish_exclusive_rebalance_request(self, request_uid: Any, *, reason: str) -> None:
        request_uid = canonical_pivotrl_request_id(request_uid)
        ...
        if not self._rebalance_pending_request_ids:
            pivotrl_logger.info("Exclusive RM rebalance completed; resuming normal routing.")
        self._signal_routing_update()
    ...
    def _enqueue_exclusive_rebalance_request(self, request_key: str, request: DataProto) -> bool:
        ...
        pending_uids = [
            ...
            if canonical_pivotrl_request_id(uid) in self._rebalance_pending_request_ids
        ]
        if not pending_uids:
            return False
        destinations = {
    ...
    def prepare_request_migrations(
        ...
        for migration in request_migrations or []:
            ...
            logical_request_uid = canonical_pivotrl_request_id(engine_request_id)
            try:
    ...
    def get_pending_request_count(self) -> int:
        ...
        t0 = time.monotonic()
        log_elastic_rm_backlog_diag(pivotrl_logger, "stage=RewardModelRouter_enter")
    ...
    def get_pending_request_summary(self, top_t: int | None = None) -> dict[str, int]:
        ...
        t0 = time.monotonic()
        log_elastic_rm_backlog_diag(pivotrl_logger, "stage=RewardModelRouter_summary_enter")
    ...
    async def get_candidate_evaluation_snapshot(self, top_t: int | None = None) -> dict:
        ...
        for instance_id in sorted(self.request_counts):
            ...
            for request_uid in sorted(set(prompt_map) | set(response_map)):
                request = request_by_uid.get(canonical_pivotrl_request_id(request_uid))
    ...
    def _format_request_uids(request: DataProto) -> str:
        uid_values = PivotRL_RewardModelRouter._request_uid_values(request)
        if not uid_values:
            return "unknown"
...
def launch_router_process(
    ...
    Args:
        ...
        config: Full PivotRL Hydra config for the router actor.
    ...
    """
    del max_attempts
    router_handle = PivotRL_RewardModelRouter.options(max_concurrency=max_concurrency).remote(
    ...
    pivotrl_logger.info("RewardModelRouter launched as Ray actor")
