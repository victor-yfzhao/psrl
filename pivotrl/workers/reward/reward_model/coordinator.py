...
class RewardModelCoordinator(CommandExtension):
    def __init__(
        ...
        Coordinates and manages reward model instances for PivotRL.
    ...
    def set_reward_model_wg_list(self, reward_model_wg_list):
        ...
        pivotrl_logger.info(f"Set reward model worker group list for {self.reward_model_name}, size: {self.reward_model_wg_size}")

    def set_reward_model_router(self, reward_model_router):
        self.reward_model_router = reward_model_router
        pivotrl_logger.info("Set reward model router for %s", self.reward_model_name)
    ...
    async def init_model(self):
        ...
        pivotrl_logger.info(f"Reward model {self.reward_model_name} initialized.")

        if self.config.pivotrl.deployment.elastic_rm.enable:
            # Only rank-0 workers own vLLM AsyncLLM when TP/PP > 1 (see PivotRL_vLLMRollout).
            sleep_futures = []
            for i in range(self.reward_model_wg_size):
                sleep_futures.append(self.reward_model_wg_list[i].execute_rank_zero_async("sleep"))
            await asyncio.gather(*sleep_futures)
            pivotrl_logger.info(f"Reward model {self.reward_model_name} sleeping.")
    ...
    async def _command_handler_loop(self):
            ...
            if not self.command_queue.empty():
                ...
                if command_type == CommandType.ABORT:
                    ...
                    try:
                        ...
                        pivotrl_logger.exception("SLEEP command failed for RM instances %s", instance_ids)
                        if self.reward_model_router is not None:
                    ...
                    try:
                        ...
                        pivotrl_logger.exception("WAKE_UP command failed for RM instances %s", instance_ids)
                        self._complete_command(command_id, False)
                    else:
                        success = True
        ...
        pivotrl_logger.info("Background command handler of reward model coordinator has finished.")
    ...
    async def init_route_strategy(self):
        ...
        # await self._is_init_model.wait()
        # futures = []
        # for i in range(self.config.pivotrl.deployment.n_rollout_instances):
        ...
        # pivotrl_logger.info(f"Max model lens: {max_model_lens}")
        # instance_to_max_model_len = {
        #     i: max(max_model_lens[i]) for i in range(self.config.pivotrl.deployment.n_rollout_instances)
        # }
