...
class OPDRewardLoopManager(RewardLoopManagerBase):
    ...
    def __init__(
        ...
        **reward_kwargs,
        ...
        pivotrl_logger.addHandler(DualOutputHandler(self.config.pivotrl.logging_path, "opd_reward_loop"))
        pivotrl_logger.info("Initialized OPDRewardLoopManager.")
    ...
    async def _query_teacher_model(
        ...
        if self.router_process is not None:
            pivotrl_logger.info("Routing OPD teacher request uid=%s through router.", request_uid)
            ...
            pivotrl_logger.info("Sending OPD teacher request uid=%s to replica_%d.", request_uid, replica_idx)
        ...
        if teacher_outputs is None or len(teacher_outputs) == 0:
            pivotrl_logger.warning("Teacher model returned empty output for uid=%s.", request_uid)
            pivotrl_logger.info(
