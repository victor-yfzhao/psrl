...
class SkyworkGenRewardFunction(GenRewardFunctionBase):
    ...
    def compute_score(
        ...
        **kwargs,
        ...
        pivotrl_logger.info(f"SkyworkGenRewardFunction.compute_score called with rm_output_value={rm_output_value}")
        if rm_output_value is not None:
            score = float(rm_output_value)
            pivotrl_logger.info(f"SkyworkGenRewardFunction returning score={score}")
            return score
        else:
            pivotrl_logger.warning("No rm_output_value provided for Skywork reward model, returning 0.0")
            return 0.0
